...
class AgentAnalytics:
    ...
    async def record_agent_action(
        ...
        try:
            ...
            AGENT_ACTION_COUNTER.labels(
                agent_id=agent_id, action_type=action_type, outcome=outcome
            ).inc()

            AGENT_LATENCY.labels(agent_id=agent_id, action_type=action_type).observe(
                duration_ms / 1000
            )  # Convert to seconds
    ...
    async def record_mcp_tool_usage(
        ...
        try:
            ...
            MCP_TOOL_LATENCY.labels(tool_name=tool_name, operation=operation).observe(
                duration_ms / 1000
            )  # Convert to seconds
    ...
    async def record_workflow_transition(
        ...
        try:
            ...
            WORKFLOW_STAGE_DURATION.labels(
                workflow_id=workflow_id, state=from_state
            ).observe(
                duration_ms / 1000
            )  # Convert to seconds
    ...
    async def _update_realtime_stats(
        self, key: str, action: str, outcome: str, duration_ms: float
    ):
        """
    ...
    async def get_agent_performance(
        ...
        try:
            ...
            # Query MongoDB
            results = await mongo_service.async_find_many(self.agent_collection, query)
        ...
        try:
            ...
            if metrics["total_operations"] > 0:
                metrics["avg_duration_ms"] = (
                    total_duration / metrics["total_operations"]
                )
    ...
    async def get_workflow_performance(
        ...
        try:
            ...
            if metrics["total_transitions"] > 0:
                metrics["avg_duration_ms"] = (
                    total_duration / metrics["total_transitions"]
                )
    ...
    async def clean_old_metrics(self, days_to_keep: int = None) -> Dict[str, int]:
        ...
        try:
            ...
            retention_days = (
                days_to_keep if days_to_keep is not None else self.metric_retention_days
            )
            ...
            agent_count = await mongo_service.async_delete_many(
                self.agent_collection, {"timestamp": {"$lt": cutoff_date}}
            )

            workflow_count = await mongo_service.async_delete_many(
                self.workflow_collection, {"timestamp": {"$lt": cutoff_date}}
            )

            mcp_tool_count = await mongo_service.async_delete_many(
                self.mcp_tool_collection, {"timestamp": {"$lt": cutoff_date}}
            )
