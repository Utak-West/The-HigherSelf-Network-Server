...
class BetterModeService(BaseService):
    ...
    def _load_config_from_env(self) -> BetterModeIntegrationConfig:
        ...
        return BetterModeIntegrationConfig(
            ...
            api_url=os.getenv("BETTERMODE_API_URL", "https://app.bettermode.com/api/graphql")
        )
    ...
    async def validate_connection(self) -> bool:
        ...
        try:
            ...
            if is_api_disabled("bettermode"):
                ...
                logger.info("[TESTING MODE] Simulated BetterMode API connection validation")
                return True
            ...
            return "network" in result and result["network"]["id"] == self.config.network_id
    ...
    async def _execute_graphql(self, query: str, variables: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if is_api_disabled("bettermode"):
            ...
            logger.info(f"[TESTING MODE] Simulated BetterMode GraphQL query: {query[:50]}...")
            return {"data": {}}
        ...
        if response.status_code != 200:
            logger.error(f"BetterMode API error: {response.status_code} - {response.text}")
    ...
    async def get_member(self, member_id: str) -> Optional[BetterModeMember]:
        ...
        try:
            ...
            return BetterModeMember(
                ...
                joined_at=datetime.fromisoformat(member_data["joinedAt"].replace("Z", "+00:00")),
    ...
    async def send_direct_message(self, recipient_id: str, message: str) -> bool:
        ...
        try:
            result = await self._execute_graphql(query, variables)
            return "sendDirectMessage" in result and result["sendDirectMessage"]["message"]["id"]
        except Exception as e:
            logger.error(f"Error sending BetterMode direct message to {recipient_id}: {str(e)}")
            return False
    ...
    async def create_post(
        ...
        try:
            ...
            logger.error(f"Error creating BetterMode post in space {space_id}: {str(e)}")
            return None
