...
class KnowledgeService:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                and provider_health["success"]
                and notion_health["healthy"],
            ...
            return {"healthy": False, "error": str(e), "component": "knowledge_service"}

    async def sync_notion_page_to_vector(
        self, page_id: str, force_update: bool = False
    ) -> Optional[UUID]:
        """
        ...
        try:
            ...
            if existing and not force_update:
                logger.info(
                    f"Page {page_id} already exists in vector store with the same content"
                )
            ...
            if embedding_id:
                logger.info(
                    f"Successfully synced page {page_id} to vector store: {embedding_id}"
                )
            else:
    ...
    async def sync_notion_database_to_vector(
        ...
        try:
            ...
            if modified_since:
                filter_params = {
                    "filter": {
                        ...
                        "date": {"on_or_after": modified_since.isoformat()},
                    }
                }
            ...
            for page in pages:
                page_id = page.get("id")
                embedding_id = await self.sync_notion_page_to_vector(
                    page_id, force_update
                )
                if embedding_id:
                    embedding_ids.append(embedding_id)

            logger.info(
                f"Synced {len(embedding_ids)} pages from database {database_id}"
            )
    ...
    async def sync_all_notion_databases_to_vector(
        self, modified_since: Optional[datetime] = None, force_update: bool = False
    ) -> Dict[str, List[UUID]]:
        """
    ...
    async def semantic_search_notion(
        ...
        if database_ids:
            ...
            results = await self.semantic_search.search(
                query=query, content_types=content_types, limit=limit
            )
        ...
        for result in results:
            ...
            if notion_page_id:
                ...
                try:
                    page_info = await self.notion_service.get_page_basic_info(
                        notion_page_id
                    )
                    ...
                    if page_info:
                        ...
                        result["notion_last_edited_time"] = page_info.get(
                            "last_edited_time"
                        )
                except Exception as e:
