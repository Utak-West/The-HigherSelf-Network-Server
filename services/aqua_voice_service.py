...
class AquaVoiceService:
    ...
    async def transcribe(self, request: VoiceTranscriptionRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(f"Aqua Voice API error: {response.status_code} - {response.text}")
                return {
                    "success": False,
    ...
    async def _local_transcribe(self, request: VoiceTranscriptionRequest) -> Dict[str, Any]:
        """
    ...
    async def process_command(self, request: VoiceCommandRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            command_context = f"The user has given the following voice command: {request.text}"
            if request.context:
            ...
            completion_response = await self.ai_router.get_completion(completion_request)
            
            if not completion_response.get("success", False):
                logger.error(f"AI completion error: {completion_response.get('error', 'Unknown error')}")
                return {
                    "success": False,
    ...
    def _is_command(self, text: str) -> bool:
        ...
        command_indicators = [
            ...
            "create", "make", "add", "schedule", "set up",
            "delete", "remove", "cancel", "update", "change",
            "send", "email", "message", "call", "contact",
...
async def get_aqua_voice_service(ai_router: Optional[AIRouter] = None) -> AquaVoiceService:
