...
class TypeFormService:
    ...
    def __init__(self, personal_token: str = None, webhook_secret: str = None):
        ...
        """
        self.personal_token = personal_token or os.environ.get("TYPEFORM_PERSONAL_TOKEN")
        self.webhook_secret = webhook_secret or os.environ.get("TYPEFORM_WEBHOOK_SECRET")
    ...
    async def get_form_responses(self, form_id: str, page_size: int = 25, since: str = None) -> List[Dict[str, Any]]:
        """
    ...
    async def get_response(self, form_id: str, response_id: str) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.error(f"Error getting response {response_id} for form {form_id}: {e}")
            return None
    
    async def process_webhook_payload(self, payload: Dict[str, Any]) -> Optional[TypeFormResponse]:
        """
        ...
        try:
            ...
            if "token" not in form_response or "form_id" not in form_response:
                logger.error("Invalid TypeForm webhook payload: missing required fields")
                return None
            
            # Extract key information
            response_id = form_response.get("token")
            form_id = form_response.get("form_id")
            form_title = form_response.get("definition", {}).get("title", "Unknown Form")
            submitted_at = datetime.fromisoformat(form_response.get("submitted_at").replace("Z", "+00:00"))
            ...
            logger.info(f"Processed TypeForm webhook for response {response_id} from form {form_title}")
            return response
        except Exception as e:
    ...
    async def create_webhook(self, form_id: str, webhook_url: str, enabled: bool = True) -> Optional[str]:
        """
    ...
    async def update_response_with_notion_id(self, form_id: str, response_id: str, notion_page_id: str) -> bool:
        """
        ...
        logger.warning("TypeForm API doesn't support updating responses. Store mapping in Notion instead.")
        return False
