...
class DatabaseSyncService:
    ...
    def __init__(
        self, notion_service: NotionService, supabase_service: SupabaseService
    ):
        """
        ...
        if is_api_disabled("notion") or is_api_disabled("supabase"):
            logger.warning(
                f"TESTING MODE ACTIVE for {self.__class__.__name__}: Some sync operations will be simulated"
            )

    async def sync_notion_to_supabase(
        self, model_class: Type[BaseModel], notion_page_id: str
    ) -> SyncResult:
        """
        ...
        ).info("Attempting to sync record from Notion to Supabase.")
        ...
        logger.bind(
            ...
            f"Testing mode status: Notion API disabled: {notion_api_disabled}, Supabase API disabled: {supabase_api_disabled}"
        )
        ...
        if notion_api_disabled and supabase_api_disabled:
            logger.bind(
                ...
                "Both Notion and Supabase APIs are disabled in testing mode. Returning mock success result."
            )
            return SyncResult(
                success=True,
                model_name=model_name,
        ...
        try:
            ...
            if notion_api_disabled:
                logger.bind(
                    ...
                    notion_page_id=notion_page_id,
                    operation="sync_notion_to_supabase",
                ).info("Notion API is disabled in testing mode. Using mock record.")
                # Create a mock record for testing
                notion_record = (
                    model_class.model_construct()
                )  # Create an empty instance
                setattr(notion_record, "page_id", notion_page_id)
                setattr(notion_record, "id", "mock_id")
            else:
                notion_record = await self.notion_service.get_page(
                    notion_page_id, model_class
                )
            ...
            if not notion_record:
                logger.bind(
                    ...
                    notion_page_id=notion_page_id,
                    operation="sync_notion_to_supabase",
                ).warning("Record not found in Notion.")
                return SyncResult(
                    success=False,
                    model_name=model_name,
                    notion_page_id=notion_page_id,
            ...
            logger.bind(
                ...
                f"Successfully fetched record from Notion: {notion_record.model_dump_json(indent=2, exclude_none=True) if hasattr(notion_record, 'model_dump_json') else 'Mock record'}"
            )
            ...
            if supabase_api_disabled:
                logger.bind(
                    ...
                    "Supabase API is disabled in testing mode. Assuming record does not exist."
                )
            else:
            ...
            logger.bind(
                ...
                f"Found {len(existing_records)} existing records in Supabase for notion_page_id {notion_page_id}."
            )
            ...
            if existing_records:
                ...
                logger.bind(
                    ...
                    operation="sync_notion_to_supabase",
                ).info("Record exists in Supabase. Attempting update.")
                ...
                if not supabase_api_disabled:
                    ...
                    logger.bind(
                        ...
                        "Supabase API is disabled in testing mode. Simulating successful update."
                    )
                ...
                logger.bind(
                    ...
                    supabase_id=supabase_id,
                    operation="sync_notion_to_supabase",
                ).info(f"Supabase update {'succeeded' if success else 'failed'}.")
                ...
                logger.bind(
                    ...
                    notion_page_id=notion_page_id,
                    operation="sync_notion_to_supabase",
                ).info("Record does not exist in Supabase. Attempting create.")

                supabase_id = "mock_supabase_id"
                if not supabase_api_disabled:
                    supabase_id = await self.supabase_service.create_record(
                        table_name=table_name, model=notion_record
                    )
                else:
                    logger.bind(
                        model_name=model_name,
                        ...
                        "Supabase API is disabled in testing mode. Simulating successful create."
                    )
                ...
                logger.bind(
                    ...
                    f"Supabase create {'succeeded' if supabase_id else 'failed'}. New Supabase ID: {supabase_id}"
                )
            ...
            logger.bind(
                ...
                f"Exception during sync from Notion to Supabase: {e}", exc_info=True
            )
            return SyncResult(
                success=False,
                model_name=model_name,
    ...
    async def sync_supabase_to_notion(
        self, model_class: Type[BaseModel], supabase_id: str
    ) -> SyncResult:
        """
        ...
        ).info("Attempting to sync record from Supabase to Notion.")
        table_name = MODEL_TO_TABLE_MAPPING.get(model_name)
        ...
        if not table_name:
            logger.bind(
                ...
                supabase_id=supabase_id,
                operation="sync_supabase_to_notion",
            ).error(f"No table mapping found for model: {model_name}")
            return SyncResult(
                success=False,
                model_name=model_name,
                supabase_id=supabase_id,
        ...
        try:
            ...
            if supabase_api_disabled:
                logger.bind(
                    ...
                    supabase_id=supabase_id,
                    operation="sync_supabase_to_notion",
                ).info("Supabase API is disabled in testing mode. Using mock record.")
                # Create a mock record for testing
                supabase_record = (
                    model_class.model_construct()
                )  # Create an empty instance
            ...
            if not supabase_record:
                logger.bind(
                    ...
                    supabase_id=supabase_id,
                    operation="sync_supabase_to_notion",
                ).warning("Record not found in Supabase.")
                return SyncResult(
                    success=False,
                    model_name=model_name,
                    supabase_id=supabase_id,
            ...
            logger.bind(
                ...
                f"Successfully fetched record from Supabase: {supabase_record.model_dump_json(indent=2, exclude_none=True) if hasattr(supabase_record, 'model_dump_json') else 'Mock record'}"
            )
            ...
            logger.bind(
                ...
                notion_page_id=notion_page_id,
                operation="sync_supabase_to_notion",
            ).debug(f"Supabase record has notion_page_id: {notion_page_id}")
            ...
            if notion_page_id:
                ...
                logger.bind(
                    ...
                    notion_page_id=notion_page_id,
                    operation="sync_supabase_to_notion",
                ).info("Record has Notion page ID. Attempting update in Notion.")
                ...
                if not notion_api_disabled:
                    ...
                    logger.bind(
                        ...
                        "Notion API is disabled in testing mode. Simulating successful update."
                    )
                ...
                logger.bind(
                    ...
                    notion_page_id=notion_page_id,
                    operation="sync_supabase_to_notion",
                ).info(f"Notion page update {'succeeded' if success else 'failed'}.")
                ...
                logger.bind(
                    ...
                    "Record does not have Notion page ID. Attempting create in Notion."
                )

                notion_page_id = "mock_notion_page_id"
                if not notion_api_disabled:
                    notion_page_id = await self.notion_service.create_page(
                        supabase_record
                    )
                else:
                    logger.bind(
                        ...
                        "Notion API is disabled in testing mode. Simulating successful page creation."
                    )

                logger.bind(
                    model_name=model_name,
                    supabase_id=supabase_id,
                    new_notion_page_id=notion_page_id,
                    operation="sync_supabase_to_notion",
                ).info(
                    f"Notion page create {'succeeded' if notion_page_id else 'failed'}. New Notion Page ID: {notion_page_id}"
                )
                ...
                if notion_page_id:
                    ...
                    logger.bind(
                        ...
                        new_notion_page_id=notion_page_id,
                        operation="sync_supabase_to_notion",
                    ).info("Updating Supabase record with new Notion page ID.")
                    ...
                    if not supabase_api_disabled:
                        ...
                        logger.bind(
                            ...
                            "Supabase API is disabled in testing mode. Simulating successful record update."
                        )
                    ...
                    logger.bind(
                        ...
                        f"Supabase record update with new Notion page ID {'succeeded' if update_success else 'failed'}."
                    )
            ...
            logger.bind(
                ...
                f"Exception during sync from Supabase to Notion: {e}", exc_info=True
            )
            return SyncResult(
                success=False,
                model_name=model_name,
    ...
    async def sync_all_records(
        ...
        model_name = model_class.__name__
        logger.info(
            f"--- DatabaseSyncService.sync_all_records START for {model_name} ---"
        )  # ADDED FOR DEBUGGING
        logger.bind(
            model_name=model_name,
            direction=direction,
        ...
        ).info("Attempting to sync all records for model.")
        table_name = MODEL_TO_TABLE_MAPPING.get(model_name)
        results = []

        if not table_name:
            logger.bind(model_name=model_name, operation="sync_all_records").error(
                f"No table mapping found for model: {model_name}"
            )
            results.append(
                SyncResult(
                    success=False,
        ...
        try:
            ...
            if not effective_since:
                effective_since = self.last_sync_timestamps.get(
                    model_name, datetime.now() - timedelta(days=30)
                )
                logger.bind(model_name=model_name, operation="sync_all_records").info(
                    f"No 'since' timestamp provided, using effective_since: {effective_since.isoformat()}"
                )
            else:
                logger.bind(model_name=model_name, operation="sync_all_records").info(
                    f"Using provided 'since' timestamp: {effective_since.isoformat()}"
                )
            ...
            if direction in ["notion_to_supabase", "both"]:
                logger.bind(
                    model_name=model_name,
                    operation="sync_all_records",
                    sub_operation="notion_to_supabase",
                ).info("Starting sync from Notion to Supabase.")
                ...
                if effective_since:
                    ...
                    logger.bind(
                        ...
                        f"Filtering Notion records updated on_or_after: {effective_since.isoformat()}"
                    )
                ...
                if notion_api_disabled:
                    ...
                    logger.bind(
                        ...
                        operation="sync_all_records",
                        sub_operation="notion_to_supabase",
                    ).info("Notion API is disabled in testing mode. Using mock data.")
                ...
                logger.bind(
                    ...
                    operation="sync_all_records",
                    sub_operation="notion_to_supabase",
                ).info(f"Retrieved {len(notion_records)} records from Notion.")
                ...
                for i, record in enumerate(notion_records):
                    ...
                    logger.bind(
                        ...
                        notion_page_id=notion_page_id,
                        item_index=i,
                        total_items=len(notion_records),
                        operation="sync_all_records",
                        sub_operation="notion_to_supabase_item",
                    ).debug("Processing Notion record.")
                    if notion_page_id:
                        result = await self.sync_notion_to_supabase(
                            model_class, notion_page_id
                        )
                        ...
                        logger.bind(
                            ...
                            f"Sync result: {result.success}, Error: {result.error_message}"
                        )
            ...
            if direction in ["supabase_to_notion", "both"]:
                logger.bind(
                    model_name=model_name,
                    operation="sync_all_records",
                    sub_operation="supabase_to_notion",
                ).info("Starting sync from Supabase to Notion.")
                # Create filter for records updated since the specified timestamp
                filters = None
                if effective_since:
                    ...
                    filters = {"updated_at": f"gte.{effective_since.isoformat()}"}
                    logger.bind(
                        model_name=model_name,
                        operation="sync_all_records",
                        sub_operation="supabase_to_notion",
                    ).info(
                        f"Filtering Supabase records updated gte: {effective_since.isoformat()}"
                    )
                ...
                if supabase_api_disabled:
                    ...
                    logger.bind(
                        ...
                        operation="sync_all_records",
                        sub_operation="supabase_to_notion",
                    ).info("Supabase API is disabled in testing mode. Using mock data.")
                    # No mock records for now, just an empty list
                else:
                ...
                logger.bind(
                    ...
                    operation="sync_all_records",
                    sub_operation="supabase_to_notion",
                ).info(f"Retrieved {len(supabase_records)} records from Supabase.")
                ...
                for i, record in enumerate(supabase_records):
                    ...
                    logger.bind(
                        ...
                        notion_page_id=notion_page_id,
                        item_index=i,
                        total_items=len(supabase_records),
                        operation="sync_all_records",
                        sub_operation="supabase_to_notion_item",
                    ).debug("Processing Supabase record.")
                    ...
                    if (
                        supabase_id and not notion_page_id
                    ):  # Simplified logic: only sync if Notion page ID is missing
                        logger.bind(
                            model_name=model_name,
                            supabase_id=supabase_id,
                            item_index=i,
                            ...
                            "Supabase record missing notion_page_id, attempting sync to Notion."
                        )
                        result = await self.sync_supabase_to_notion(
                            model_class, supabase_id
                        )
                        results.append(result)
                        logger.bind(
                            model_name=model_name,
                        ...
                        logger.bind(
                            ...
                            "Supabase record already has notion_page_id, skipping sync to Notion to avoid potential loops without further logic."
                        )
            ...
            logger.bind(
                model_name=model_name,
                new_sync_timestamp=new_sync_time.isoformat(),
                operation="sync_all_records",
            ).info("Updated last_sync_timestamp for model.")

            return results
        except Exception as e:
            logger.bind(model_name=model_name, operation="sync_all_records").error(
                f"Exception during sync_all_records for {model_name}: {e}",
                exc_info=True,
            )
            results.append(
                SyncResult(success=False, model_name=model_name, error_message=str(e))
            )
            return results

    async def sync_all_databases(
        self, direction: str = "both", since: Optional[datetime] = None
        ...
        from models.notion_db_models import (Agent, AgentCommunication,
                                             ApiIntegration, BusinessEntity,
                                             DataTransformation,
                                             NotificationTemplate, Task,
                                             UseCase, Workflow,
                                             WorkflowInstance)
        ...
        for model_class in models:
            ...
            logger.info(
                f"Synced {model_name}: {success_count}/{total_count} successful"
            )
    ...
    async def create_from_services(
        cls, notion_service: NotionService, supabase_service: SupabaseService
    ) -> "DatabaseSyncService":
        """
