...
class BaseService:
    ...
    def __init__(
        self, service_name: str, credentials: Optional[ServiceCredentials] = None
    ):
        """
        ...
        self.max_retries = int(
            os.environ.get(f"{service_name.upper()}_MAX_RETRIES", "3")
        )
        self.base_retry_delay = float(
            os.environ.get(f"{service_name.upper()}_RETRY_DELAY", "1.0")
        )
        self.max_retry_delay = float(
            os.environ.get(f"{service_name.upper()}_MAX_RETRY_DELAY", "60.0")
        )
        ...
        while True:
            try:
                ...
                if retries > max_retries:
                    logger.error(
                        f"{self.service_name} API call failed after {max_retries} retries: {e}"
                    )
                    raise
                ...
                delay = min(
                    ...
                    base_delay
                    * (2 ** (retries - 1))
                    * (0.9 + 0.2 * asyncio.get_event_loop().time() % 1),
                )

                logger.warning(
                    f"{self.service_name} API call failed: {e}. Retry {retries}/{max_retries} after {delay:.2f}s"
                )
        ...
        **kwargs,
        ...
        async def _make_request():
            async with session.post(
                url, data=data, json=json_data, headers=headers, **kwargs
            ) as response:
                response.raise_for_status()
                return await response.json()
    ...
    async def rotate_credentials(self) -> bool:
        ...
        """
        logger.warning(
            f"Credential rotation not implemented for {self.service_name} service"
        )
        return False
        ...
        if timestamp:
            try:
                ...
                if datetime.now() - webhook_time > timedelta(minutes=5):
                    logger.warning(
                        f"{self.service_name} webhook timestamp too old: {timestamp}"
                    )
                    return False
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "credentials_verified": (
                self.credentials.last_verified.isoformat()
                if self.credentials and self.credentials.last_verified
                else None
            ),
            "last_api_call": (
            ...
            "uptime_seconds": (
                (now - self.initialized_at).total_seconds()
            ...
            "error_rate": (
                self.error_count / self.api_call_count if self.api_call_count > 0 else 0
            ),
        }
    ...
    def validate_model(model: BaseModel) -> None:
        ...
        try:
            ...
            for field_name, field_value in model_dict.items():
                # Check required fields using model_fields instead of __required_fields__
                model_fields = getattr(model.__class__, "model_fields", {})
                field_info = model_fields.get(field_name, None)
                if field_value is None and field_info and not field_info.is_optional():
                    raise ValueError(
