...
from models.capcut_models import (
    ...
    CapCutWebhookPayload, CapCutVideoMetadata, CapCutExportStatus
)
...
class CapCutService(BaseService):
    ...
    def __init__(
        ...
        self.base_url = base_url or os.environ.get("CAPCUT_BASE_URL", "https://api.capcut.com/v1")
    ...
    async def export_video(self, request: CapCutExportRequest) -> CapCutExportResponse:
        ...
        try:
            ...
            if self.notion_service:
                ...
                # Save to Notion
                content_id = await self.notion_service.create_video_content(video_content)
            ...
            return CapCutExportResponse(
                ...
                estimated_completion_time=datetime.fromisoformat(data.get("estimated_completion_time")) if data.get("estimated_completion_time") else None
            )
        
        except httpx.HTTPStatusError as e:
            logger.error(f"HTTP error exporting video: {e.response.status_code} - {e.response.text}")
            return CapCutExportResponse(
                status="error",
    ...
    async def get_export_status(self, export_id: str) -> CapCutExportStatusResponse:
        ...
        try:
            ...
            if self.notion_service:
                ...
                content = await self.notion_service.find_video_content_by_task_id(export_id)
                if content:
                    content_id = content.id
                    
                    # Update video content if export is completed
                    if data.get("status") == CapCutExportStatus.COMPLETED.value and data.get("video_url"):
                        ...
                        if metadata:
                            updates["title"] = metadata.title if metadata.title else content.title
                            updates["description"] = metadata.description if metadata.description else content.description
                            updates["duration"] = metadata.duration
                            updates["resolution"] = f"{metadata.width}x{metadata.height}"
                            updates["tags"] = metadata.tags
                        
                        # Save updates to Notion
                        await self.notion_service.update_video_content(content_id, updates)
                        logger.info(f"Updated video content in Notion with export results: {content_id}")
            ...
            logger.error(f"HTTP error checking export status: {e.response.status_code} - {e.response.text}")
            return CapCutExportStatusResponse(
                status="error",
                export_id=export_id,
    ...
    async def process_webhook(self, payload: Dict[str, Any], signature: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.notion_service:
                # Query Notion for video content with this export ID as task_id
                content = await self.notion_service.find_video_content_by_task_id(webhook_data.export_id)
                if content:
                    # Update with video URL and status
                    updates = {
                        "video_status": VideoStatus.COMPLETED.value if webhook_data.status == CapCutExportStatus.COMPLETED else VideoStatus.FAILED.value
                    }
                    ...
                    if webhook_data.metadata:
                        updates["title"] = webhook_data.metadata.title if webhook_data.metadata.title else content.title
                        updates["description"] = webhook_data.metadata.description if webhook_data.metadata.description else content.description
                        updates["duration"] = webhook_data.metadata.duration
                        updates["resolution"] = f"{webhook_data.metadata.width}x{webhook_data.metadata.height}"
                        updates["tags"] = webhook_data.metadata.tags
                    
                    # Save updates to Notion
                    await self.notion_service.update_video_content(content.id, updates)
                    logger.info(f"Updated video content in Notion from webhook: {content.id}")
