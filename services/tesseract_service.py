...
from services.cache_service import multi_level_cache, CacheType
...
class TesseractService:
    ...
    def __init__(
        ...
        if not TESSERACT_AVAILABLE:
            logger.warning("TesseractService initialized but pytesseract is not installed.")
            return
        ...
        try:
            ...
            if self.cache_results:
                ...
                await multi_level_cache.set(
                    cache_key,
                    result.dict(exclude={"from_cache"}),
                    CacheType.FUNCTION
                )
