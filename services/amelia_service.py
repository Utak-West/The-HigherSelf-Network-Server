...
class AmeliaServiceClient:
    ...
    async def get_appointments(self, 
                              ...
                              status: str = None) -> List[AmeliaAppointment]:
        """
                ...
                try:
                    ...
                    if "customFields" in data:
                        for field in data["customFields"]:
                            ...
                            if field.get("name") == "notion_page_id" or field.get("label") == "Notion Page ID":
                                notion_page_id = field.get("value")
                    
                    # Parse the appointment data
                    appointment = AmeliaAppointment(
                        id=data.get("id"),
                        booking_start=datetime.fromisoformat(data.get("bookingStart").replace("Z", "+00:00")),
                        booking_end=datetime.fromisoformat(data.get("bookingEnd").replace("Z", "+00:00")),
                        ...
                        payment_amount=float(data.get("payment", {}).get("amount", 0)) if "payment" in data else None,
                        payment_status=data.get("payment", {}).get("status") if "payment" in data else None,
                    ...
                    if "created" in data:
                        appointment.created_date = datetime.fromisoformat(data.get("created").replace("Z", "+00:00"))
                    
                    if "updated" in data:
                        appointment.updated_date = datetime.fromisoformat(data.get("updated").replace("Z", "+00:00"))
    ...
    async def create_appointment(self, appointment: AmeliaAppointment) -> Optional[int]:
        ...
        try:
            ...
            if appointment.notion_page_id and not any(f.get("name") == "notion_page_id" for f in custom_fields):
                custom_fields.append({
                    "name": "notion_page_id",
    ...
    async def sync_customer_to_notion(self, customer_id: int, notion_page_id: str) -> bool:
        ...
        try:
            ...
            if not customer:
                logger.error(f"Could not find customer {customer_id} to sync with Notion")
                return False
    ...
    async def process_webhook(self, webhook_data: Dict[str, Any]) -> Optional[AmeliaAppointment]:
        """
        ...
        try:
            ...
            logger.info(f"Processed Amelia webhook: {event_type} for appointment {appointment_id}")
            return appointment
        except Exception as e:
