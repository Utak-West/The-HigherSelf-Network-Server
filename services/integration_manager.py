...
from services.amelia_service import AmeliaService
...
class IntegrationManager:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            # UserFeedback
            if self.config.enable_userfeedback:
                logger.info("Initializing UserFeedback service...")
                self.services["userfeedback"] = UserFeedbackService()
                # UserFeedback doesn't have a specific validation method
                self.initialization_status["userfeedback"] = True
            
            # TutorLM
            if self.config.enable_tutorlm:
                logger.info("Initializing TutorLM service...")
                self.services["tutorlm"] = TutorLMService()
                # TutorLM doesn't have a specific validation method
                self.initialization_status["tutorlm"] = True
