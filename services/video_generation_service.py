...
class VideoGenerationRequest(BaseModel):
    ...
    business_entity_id: Optional[str] = Field(
        None, description="Business entity ID for tracking"
    )
...
class VideoGenerationResponse(BaseModel):
    ...
    message: Optional[str] = Field(
        None, description="Additional message about the task"
    )
...
class VideoGenerationService:
    ...
    def __init__(self, api_url: Optional[str] = None):
        ...
        self.logger.info(
            f"Video Generation Service initialized with API URL: {self.api_url}"
        )
        ...
        try:
            async with httpx.AsyncClient(timeout=60.0) as client:
                response = await client.post(
                    f"{self.api_url}/generate", json=request.dict(exclude_none=True)
                )
                ...
                self.logger.info(
                    f"Video generation task created: {result.get('task_id')}"
                )
            ...
            return VideoGenerationResponse(
                task_id="error", status="failed", message=f"Service error: {str(e)}"
            )
    ...
    async def get_task_status(self, task_id: str) -> VideoTaskStatus:
        ...
        try:
            ...
            return VideoTaskStatus(
                task_id=task_id, status="error", error=f"Service error: {str(e)}"
            )
    ...
    async def get_available_voices(self) -> List[Dict[str, Any]]:
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code != 200:
                    self.logger.error(
                        f"Error getting available voices: {response.text}"
                    )
                    return []
