...
class EscalationService:
    ...
    async def create_escalation_ticket(self, escalation_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            logger.info(f"Created escalation ticket {ticket.ticket_id} for {ticket.customer_email}")
    ...
    async def _create_notion_escalation_record(self, ticket: EscalationTicket) -> Optional[str]:
    ...
    async def _send_escalation_notification(self, ticket: EscalationTicket) -> bool:
        ...
        try:
            if not all([self.smtp_server, self.smtp_username, self.smtp_password, self.notification_email]):
                logger.warning("Email configuration incomplete - escalation notification not sent")
                return False
            ...
            msg['Subject'] = f"[URGENT - Grace Fields] Human intervention required - Ticket #{ticket.ticket_id}"
