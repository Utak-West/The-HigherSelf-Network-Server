...
from models.pipit_models import (
    ...
    PipitWebhookPayload, PipitPaymentStatus, PipitCurrency
)
from models.video_transaction_models import (
    VideoTransaction, VideoTransactionStatus, VideoTransactionType
)
...
class PipitService(BaseService):
    ...
    def __init__(
        ...
        self.base_url = base_url or os.environ.get("PIPIT_BASE_URL", "https://api.pipit.com/v1")
    ...
    async def create_payment(self, request: PipitPaymentRequest) -> PipitPaymentResponse:
        """
        ...
        """
        logger.info(f"Creating Pipit payment for amount: {request.amount} {request.currency}")
        ...
        try:
            ...
            if self.notion_service:
                ...
                video_id = request.metadata.get("video_id") if request.metadata else None
                ...
                if video_id:
                    ...
                    # Save to Notion
                    transaction_id = await self.notion_service.create_video_transaction(transaction)
    ...
    async def get_payment_status(self, payment_id: str) -> PipitPaymentStatusResponse:
        ...
        try:
            ...
            if self.notion_service:
                ...
                transaction = await self.notion_service.find_transaction_by_payment_id(payment_id)
                if transaction:
                    ...
                    if transaction.payment_status != data.get("status"):
                        ...
                        elif data.get("status") in [PipitPaymentStatus.FAILED, PipitPaymentStatus.DISPUTED]:
                        ...
                        await self.notion_service.update_video_transaction(transaction.id, updates)
                        logger.info(f"Updated video transaction in Notion with payment status: {transaction.id}")
            ...
            logger.error(f"HTTP error checking payment status: {e.response.status_code} - {e.response.text}")
            return PipitPaymentStatusResponse(
                status="error",
                payment_id=payment_id,
    ...
    async def process_webhook(self, payload: Dict[str, Any], signature: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.notion_service:
                # Query Notion for transaction with this payment ID
                transaction = await self.notion_service.find_transaction_by_payment_id(webhook_data.payment_id)
                if transaction:
                    # Map payment status to transaction status
                    transaction_status = VideoTransactionStatus.PENDING
                    if webhook_data.payment_status == PipitPaymentStatus.COMPLETED:
                        transaction_status = VideoTransactionStatus.COMPLETED
                    elif webhook_data.payment_status in [PipitPaymentStatus.FAILED, PipitPaymentStatus.DISPUTED]:
                        transaction_status = VideoTransactionStatus.FAILED
                    ...
                    await self.notion_service.update_video_transaction(transaction.id, updates)
                    logger.info(f"Updated video transaction in Notion from webhook: {transaction.id}")
