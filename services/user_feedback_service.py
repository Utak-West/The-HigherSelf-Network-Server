...
class UserFeedbackService(BaseService):
    ...
    def __init__(self, api_key: str = None):
        ...
        self.base_url = os.environ.get(
            "USER_FEEDBACK_API_URL", "https://api.userfeedback.example.com"
        )
    ...
    async def submit_feedback(self, feedback: UserFeedbackItem) -> Optional[str]:
        ...
        try:
            ...
            response = await self.async_post(
                f"{self.base_url}/api/feedback", headers=headers, json=feedback_data
            )
    ...
    async def get_feedback(self, feedback_id: str) -> Optional[UserFeedbackItem]:
        ...
        try:
            headers = {"Authorization": f"Bearer {self.api_key}"}
            response = await self.async_get(
                f"{self.base_url}/api/feedback/{feedback_id}", headers=headers
            )
            ...
            if response.get("created_at"):
                try:
                    ...
                    logger.warning(
                        f"Could not parse created_at from feedback {feedback_id}"
                    )
        ...
        try:
            ...
            if signature and self.credentials and self.credentials.webhook_secret:
                if not self._verify_webhook_signature(
                    payload, signature, self.credentials.webhook_secret
                ):
            ...
            if event_type == "feedback.created":
                ...
                if feedback_id:
                    ...
                    if feedback:
                        ...
                        notion_page_id = await self._create_notion_feedback_record(
                            feedback
                        )
                        if notion_page_id:
                            # Update feedback with Notion page ID
                            feedback.notion_page_id = notion_page_id
                            await self._update_feedback_with_notion_id(
                                feedback_id, notion_page_id
                            )
                            logger.info(
                                f"Processed new feedback: {feedback_id}, Notion page: {notion_page_id}"
                            )
                            return True
                        else:
                            logger.warning(
                                f"Failed to create Notion record for feedback: {feedback_id}"
                            )
                            return False
                ...
                if feedback_id:
                    ...
                    if feedback:
                        ...
                        if feedback.notion_page_id:
                            ...
                            success = await self._update_notion_feedback_record(
                                feedback
                            )
                            if success:
                                logger.info(
                                    f"Updated Notion record for feedback: {feedback_id}"
                                )
                                return True
                            else:
                                logger.warning(
                                    f"Failed to update Notion record for feedback: {feedback_id}"
                                )
                                return False
                        else:
                            ...
                            if notion_page_id:
                                ...
                                logger.info(
                                    f"Created Notion record for updated feedback: {feedback_id}"
                                )
                                return True
                            else:
                                logger.warning(
                                    f"Failed to create Notion record for updated feedback: {feedback_id}"
                                )
                                return False
    ...
    async def _create_notion_feedback_record(
        self, feedback: UserFeedbackItem
    ) -> Optional[str]:
        """
    ...
    async def _update_notion_feedback_record(self, feedback: UserFeedbackItem) -> bool:
        ...
        try:
            ...
            if not feedback.notion_page_id:
                logger.warning(
                    "Cannot update Notion record: No notion_page_id provided"
                )
                return False
            ...
            existing_records = await notion_service.get_page(
                model_class=FeedbackSurvey, page_id=feedback.notion_page_id
            )
            ...
            existing_record.user_email = (
                feedback.user_email or existing_record.user_email
            )
            ...
            if success:
                logger.info(
                    f"Updated Notion feedback record: {feedback.notion_page_id}"
                )
            else:
                logger.warning(
                    f"Failed to update Notion feedback record: {feedback.notion_page_id}"
                )
    ...
    async def _update_feedback_with_notion_id(
        self, feedback_id: str, notion_page_id: str
    ) -> bool:
        """
        ...
        try:
            ...
            if response and response.get("id") == feedback_id:
                logger.info(
                    f"Updated feedback {feedback_id} with Notion page ID: {notion_page_id}"
                )
                return True
            else:
                logger.warning(
                    f"Failed to update feedback {feedback_id} with Notion page ID"
                )
                return False
        except Exception as e:
