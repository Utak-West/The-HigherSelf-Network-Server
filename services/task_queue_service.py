...
def handle_task_exception(task_id, exception, traceback, task_name):
    ...
    try:
        ...
        mongo_service.insert_one("task_errors", {
            "task_id": task_id,
            "task_name": task_name,
            "error": str(exception),
            "traceback": traceback,
...
def log_task_success(task_id, result, task_name, duration):
    ...
    if duration > 10:  # Only log tasks that took more than 10 seconds
        logger.info(f"Task {task_name} (ID: {task_id}) completed successfully in {duration:.2f}s")
        
        # Record in MongoDB for analytics
        try:
            from services.mongodb_service import mongo_service
            mongo_service.insert_one("task_performance", {
                "task_id": task_id,
                "task_name": task_name,
                "duration": duration,
                "timestamp": time.time()
