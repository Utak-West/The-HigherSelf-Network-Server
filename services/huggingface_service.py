...
class HuggingFaceService(BaseService):
    ...
    async def query_model(self, model_config: HuggingFaceModelConfig, inputs: str) -> Dict[str, Any]:
        """
    ...
    async def process_notion_integration(
        ...
        try:
            ...
            if not notion_data:
                raise ValueError("Notion data not provided and Notion service not available")
            ...
            if input_prop in notion_data:
                ...
                if "rich_text" in prop_data:
                    input_text = "".join([text.get("plain_text", "") for text in prop_data["rich_text"]])
                elif "title" in prop_data:
                    input_text = "".join([text.get("plain_text", "") for text in prop_data["title"]])
            ...
            if not input_text:
                ...
                if workflow_id and self.notion_service:
                    await self.notion_service.log_to_workflow_history(workflow_id, "ERROR", error_msg)
            ...
            if self.notion_service:
                await self.notion_service.update_page(
                    page_id=integration.notion_page_id,
