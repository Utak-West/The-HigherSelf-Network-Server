...
class AirtableService:
    ...
    def __init__(self, api_key: str = None, base_id: str = None):
        ...
        if not self.api_key:
            logger.warning("Airtable API key not set. Some functionality may be limited.")
        
        if not self.base_id:
            logger.warning("Airtable base ID not set. Some functionality may be limited.")
    ...
    async def create_record(self, table_name: str, fields: Dict[str, Any]) -> Optional[str]:
        """
    ...
    async def get_record(self, table_name: str, record_id: str) -> Optional[AirtableRecord]:
        """
    ...
    async def update_record(self, table_name: str, record_id: str, fields: Dict[str, Any]) -> bool:
        """
    ...
    async def list_records(self, table_name: str, max_records: int = 100, 
                          filter_by_formula: Optional[str] = None) -> List[AirtableRecord]:
        """
        ...
        try:
            ...
            return [AirtableRecord(id=r.get("id"), fields=r.get("fields", {})) for r in records]
        except Exception as e:
