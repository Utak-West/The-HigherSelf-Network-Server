...
try:
    ...
    logger.warning("google-cloud-vision not installed. Google Vision OCR functionality will be limited.")
...
class GoogleVisionOCRService(BaseOCRService):
    ...
    def __init__(
        ...
        if not GOOGLE_VISION_AVAILABLE:
            logger.warning("GoogleVisionOCRService initialized but google-cloud-vision is not installed.")
            return
    ...
    async def process_image(self, request: OCRRequest) -> OCRResponse:
        ...
        try:
            ...
            result = OCRResponse(
                ...
                processing_time=time.time() - start_time,
                metadata={
                    "locale": response.full_text_annotation.pages[0].property.detected_languages[0].language_code if response.full_text_annotation and response.full_text_annotation.pages else None
                }
            )
    ...
    async def _load_image(self, source_type: OCRImageSource, image_data: Union[str, bytes]) -> vision.Image:
        """
