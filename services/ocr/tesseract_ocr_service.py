...
class TesseractOCRService(BaseOCRService):
    ...
    def __init__(
        ...
        if not TESSERACT_AVAILABLE:
            logger.warning("TesseractOCRService initialized but pytesseract is not installed.")
            return
    ...
    @classmethod
    def from_env(cls) -> "TesseractOCRService":
        """
        Create a TesseractOCRService instance from environment variables.
    ...
    async def process_image(self, request: OCRRequest) -> OCRResponse:
        ...
        try:
            ...
            if request.output_format == OCROutputFormat.TEXT:
                ...
                # Extract HOCR
                hocr = pytesseract.image_to_pdf_or_hocr(image, lang=languages, config=config, extension='hocr')
                result = OCRResponse(
                    success=True,
                    provider=self.provider,
    ...
    async def _load_image(self, source_type: OCRImageSource, image_data: Union[str, bytes]) -> Image.Image:
        """
    ...
    def _generate_cache_key(self, request: OCRRequest) -> str:
        ...
        return f"ocr:tesseract:{source_key}:{languages}:{request.output_format}:{config_key}:{request.preprocessing}"
