...
CACHE_HITS = Counter(
    "cache_hits_total", "Cache hit count", ["cache_level", "cache_type"]
)
CACHE_MISSES = Counter(
    "cache_misses_total", "Cache miss count", ["cache_level", "cache_type"]
)
CACHE_SIZE = Gauge(
    "cache_size", "Current number of items in cache", ["cache_level", "cache_type"]
)
...
class MultiLevelCache:
    ...
    async def get(
        ...
        try:
            ...
            CACHE_LATENCY.labels(
                operation="get", cache_level=level.value, cache_type=cache_type.value
            ).observe(duration)

            if cached_value is not None:
                CACHE_HITS.labels(
                    cache_level=level.value, cache_type=cache_type.value
                ).inc()
                return cached_value
    ...
    async def set(
        ...
        try:
            ...
            ttl = (
                ...
                else self.ttl_map.get(level, 300)
            )
            ...
            if index_size > max_size:
                await self._evict_oldest(
                    index_key, max_size // 10
                )  # Evict 10% of the cache
            ...
            duration = time.time() - start_time
            CACHE_LATENCY.labels(
                operation="set", cache_level=level.value, cache_type=cache_type.value
            ).observe(duration)

            # Update cache size metric
            index_size = await redis_service.async_scard(index_key)
            CACHE_SIZE.labels(cache_level=level.value, cache_type=cache_type.value).set(
                index_size
            )
    ...
    async def delete(self, key: str, cache_type: CacheType) -> bool:
        ...
        try:
            ...
            duration = time.time() - start_time
            CACHE_LATENCY.labels(
                operation="delete", cache_level="any", cache_type=cache_type.value
            ).observe(duration)
    ...
    async def exists(self, key: str, cache_type: CacheType) -> bool:
        ...
        try:
            ...
            duration = time.time() - start_time
            CACHE_LATENCY.labels(
                operation="exists", cache_level="any", cache_type=cache_type.value
            ).observe(duration)
    ...
    async def clear_type(self, cache_type: CacheType) -> int:
        ...
        try:
            ...
            duration = time.time() - start_time
            CACHE_LATENCY.labels(
                operation="clear_type", cache_level="all", cache_type=cache_type.value
            ).observe(duration)
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for cache_type in CacheType:
                type_stats = {
                    "default_level": self.default_level_map.get(
                        cache_type, CacheLevel.L2
                    ).value,
                    "max_size": self.max_size_map.get(cache_type, 1000),
                    "levels": {},
                }
            ...
            return {"error": str(e), "levels": {}, "types": {}}
