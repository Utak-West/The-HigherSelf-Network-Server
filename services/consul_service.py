...
class ServiceDefinition(BaseModel):
    ...
    check: Optional[Dict[str, Any]] = Field(
...
class ConsulService:
    ...
    def register_service(self, service: ServiceDefinition) -> bool:
        ...
        try:
            ...
            if result:
                ...
                logger.info(
                    f"Registered service {service.name} (ID: {service_id}) with Consul"
                )
        ...
        try:
            ...
            async with session.get(url, headers=headers) as response:
                if response.status == 200:
                    ...
                    logger.error(
                        f"Failed to get service {service_name}: HTTP {response.status}"
                    )
                    return []
        except Exception as e:
            logger.error(f"Failed to get service {service_name}: {e}")
    ...
    async def async_get_healthy_service(
        ...
        try:
            ...
            async with session.get(url, headers=headers) as response:
                if response.status == 200:
                    ...
                    logger.error(
                        f"Failed to get healthy service {service_name}: HTTP {response.status}"
                    )
                    return []
        except Exception as e:
            logger.error(f"Failed to get healthy service {service_name}: {e}")
