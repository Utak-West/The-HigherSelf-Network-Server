...
from models.softr_models import StaffUser, AgentInteraction, AgentRequest, AgentResponse
...
class SoftrService(BaseService):
    ...
    async def validate_staff_user(self, user_id: str, api_key: str) -> Optional[StaffUser]:
        """
        ...
        try:
            ...
            if response.status_code != 200:
                logger.warning(f"Failed to validate staff user {user_id}: {response.status_code}")
                return None
    ...
    async def record_agent_interaction(self, interaction: AgentInteraction) -> bool:
        ...
        try:
            ...
            if response.status_code not in (200, 201):
                logger.warning(f"Failed to record agent interaction: {response.status_code}")
                return False
    ...
    async def get_staff_agent_history(self, staff_id: str, agent_id: Optional[str] = None) -> List[AgentInteraction]:
        """
        ...
        try:
            ...
            if response.status_code != 200:
                logger.warning(f"Failed to get agent interaction history: {response.status_code}")
                return []
                
            interactions_data = response.json().get("data", [])
            return [AgentInteraction(**interaction) for interaction in interactions_data]
