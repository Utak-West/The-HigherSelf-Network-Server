...
class CapCutVideoMetadata(BaseModel):
    ...
    audio_tracks: List[str] = Field(
        default_factory=list, description="Audio tracks used"
    )
...
class CapCutExportRequest(BaseModel):
    ...
    format: CapCutVideoFormat = Field(
        CapCutVideoFormat.MP4, description="Export format"
    )
    quality: CapCutVideoQuality = Field(
        CapCutVideoQuality.FULL_HD, description="Export quality"
    )
...
class CapCutExportStatusResponse(BaseModel):
    ...
    content_id: Optional[str] = Field(
        None, description="ID of the video content in Notion"
    )
    video_url: Optional[HttpUrl] = Field(
        None, description="URL to the exported video if completed"
    )
    progress: Optional[float] = Field(None, description="Progress percentage (0-100)")
    message: Optional[str] = Field(
        None, description="Additional message about the status"
    )
    metadata: Optional[CapCutVideoMetadata] = Field(
        None, description="Video metadata if available"
    )
