...
class GHLOpportunity(BaseModel):
    ...
    # Business context
    business_type: BusinessType = Field(..., description="Business type for this opportunity")
    sub_account: SubAccountType = Field(..., description="GoHighLevel sub-account")
...
class GHLCampaign(BaseModel):
    ...
    # Performance metrics
    stats: Dict[str, Any] = Field(default_factory=dict, description="Campaign statistics")
    target_contacts: List[str] = Field(default_factory=list, description="Target contact IDs")
...
class GHLAppointment(BaseModel):
    ...
    # Business context
    business_type: BusinessType = Field(..., description="Business type for this appointment")
    sub_account: SubAccountType = Field(..., description="GoHighLevel sub-account")
...
class GHLSyncStatus(BaseModel):
    ...
    entity_type: str = Field(..., description="Type of entity (contact, opportunity, etc.)")
    ...
    sync_direction: str = Field(..., description="Sync direction (ghl_to_notion, notion_to_ghl, bidirectional)")
    sync_status: str = Field("pending", description="Sync status (pending, success, error)")
    error_message: Optional[str] = Field(None, description="Error message if sync failed")
    
    # Conflict resolution
    conflict_detected: bool = Field(False, description="Whether a sync conflict was detected")
