...
class BusinessEntity(BaseModel):
    ...
    api_keys_reference: str = Field(..., description="Secure reference to API credentials")
    primary_workflows: List[str] = Field(default_factory=list, description="IDs of primary workflows")
    active_agents: List[str] = Field(default_factory=list, description="IDs of active agents")
...
class Agent(BaseModel):
    ...
    business_entity_association: List[str] = Field(default_factory=list, description="Associated business entities")
    capabilities: List[AgentCapability] = Field(default_factory=list, description="Agent capabilities")
    primary_apis_utilized: List[ApiPlatform] = Field(default_factory=list, description="Primary APIs used")
    supported_workflows: List[str] = Field(default_factory=list, description="Supported workflow IDs")
    primary_data_sources: List[str] = Field(default_factory=list, description="Primary data sources")
    primary_data_sinks: List[str] = Field(default_factory=list, description="Primary data sinks")
    runtime_environment: RuntimeEnvironment = Field(RuntimeEnvironment.DOCKER, description="Runtime environment")
...
class Workflow(BaseModel):
    ...
    states: Dict[str, Dict[str, Any]] = Field(..., description="State machine definition")
    transitions: List[Dict[str, Any]] = Field(..., description="State transitions")
    required_agents: List[str] = Field(default_factory=list, description="Required agent IDs")
    visualization_url: Optional[str] = Field(None, description="URL to state machine diagram")
...
class WorkflowInstance(BaseModel):
    ...
    """
    instance_id: str = Field(default_factory=lambda: f"INST-{uuid4().hex[:8]}", description="Unique instance ID")
    ...
    start_date: datetime = Field(default_factory=datetime.now, description="Workflow start time")
    last_transition_date: datetime = Field(default_factory=datetime.now, description="Last state transition time")
    status: InstanceStatus = Field(InstanceStatus.ACTIVE, description="Instance status")
    error_message: Optional[str] = Field(None, description="Error message if status is Error")
    assigned_to: Optional[str] = Field(None, description="User assigned to this instance")
    priority: str = Field("Medium", description="Priority level")
    tags: List[str] = Field(default_factory=list, description="Tags for filtering")
    source_system: Optional[str] = Field(None, description="Source system (e.g., Amelia, WooCommerce)")
    source_record_id: Optional[str] = Field(None, description="Record ID in source system")
    hubspot_contact_id: Optional[str] = Field(None, description="HubSpot contact ID if synced")
    airtable_record_id: Optional[str] = Field(None, description="Airtable record ID if synced")
    key_data_payload: Optional[Dict[str, Any]] = Field(None, description="Critical data in JSON format")
    history_log: List[Dict[str, Any]] = Field(default_factory=list, description="Append-only log of state transitions")
...
class ApiIntegration(BaseModel):
    ...
    authentication_type: str = Field(..., description="Auth type (e.g., OAuth, API Key)")
    credential_reference: str = Field(..., description="Secure reference to credentials")
    primary_endpoints: List[str] = Field(default_factory=list, description="Main endpoints used")
    documentation_url: Optional[str] = Field(None, description="Link to API documentation")
    business_entities: List[str] = Field(default_factory=list, description="Associated business entity IDs")
...
class DataTransformation(BaseModel):
    ...
    transformation_logic: str = Field(..., description="Logic as Python code or pseudo-code")
    ...
    used_by_workflows: List[str] = Field(default_factory=list, description="Workflow IDs using this transformation")
...
class UseCase(BaseModel):
    ...
    business_entities: List[str] = Field(default_factory=list, description="Business entity IDs")
    implemented_by_workflows: List[str] = Field(default_factory=list, description="Implementing workflow IDs")
    ...
    notion_dashboard_link: Optional[str] = Field(None, description="Link to Notion dashboard")
...
class NotificationTemplate(BaseModel):
    ...
    subject_template: Optional[str] = Field(None, description="Subject line for email templates")
    supported_placeholders: List[str] = Field(default_factory=list, description="Valid placeholder names")
    business_entities: List[str] = Field(default_factory=list, description="Business entity IDs")
...
class AgentCommunication(BaseModel):
    ...
    communication_protocol: str = Field("HTTP", description="Protocol (HTTP, Message Queue, etc.)")
    sample_payload: Dict[str, Any] = Field(..., description="Sample message payload")
    active_workflows_using: List[str] = Field(default_factory=list, description="Workflow IDs using this pattern")
...
class Task(BaseModel):
    ...
    """
    task_id: str = Field(default_factory=lambda: f"TASK-{uuid4().hex[:8]}", description="Unique task ID")
    ...
    related_workflow_instance: Optional[str] = Field(None, description="Related workflow instance ID")
    ...
    created_date: datetime = Field(default_factory=datetime.now, description="Creation date")
    last_edited_date: datetime = Field(default_factory=datetime.now, description="Last edit date")
    tags: List[str] = Field(default_factory=list, description="Tags for filtering")
...
class AIContentReview(BaseModel):
    ...
    source_ai_tool: str = Field(..., description="Source AI tool (e.g., Plaud, TutorLM)")
    ...
    status: ContentReviewStatus = Field(ContentReviewStatus.PENDING_REVIEW, description="Review status")
