...
class ModelMetadata(BaseModel):
    ...
    size_category: str = Field(
        "medium", description="Size category: tiny, small, medium, large, xlarge"
    )
    parameters: Optional[int] = Field(
        None, description="Number of parameters (millions)"
    )
    languages: List[str] = Field(
        default_factory=lambda: ["en"], description="Supported languages"
    )
    specialties: List[str] = Field(
        default_factory=list, description="Areas where this model excels"
    )
    limitations: List[str] = Field(
        default_factory=list, description="Known limitations"
    )
    default_parameters: Dict[str, Any] = Field(
        default_factory=dict, description="Default parameters for this model"
    )
    performance_metrics: Dict[str, float] = Field(
        default_factory=dict, description="Performance metrics on benchmarks"
    )

    # Resource requirements
    memory_requirements: Optional[str] = Field(
        None, description="Estimated memory requirements"
    )
    inference_speed: Optional[str] = Field(
        None, description="Inference speed category: fast, medium, slow"
    )
    ...
    not_recommended_for: List[str] = Field(
        default_factory=list, description="Use cases to avoid"
    )
