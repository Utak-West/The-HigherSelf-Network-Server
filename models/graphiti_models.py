...
class GraphitiEpisodeRequest(BaseModel):
    ...
    source: GraphitiEpisodeType = Field(
        default=GraphitiEpisodeType.TEXT,
    ...
    agent_name: Optional[GraphitiAgentName] = Field(
        default=None,
        description="Name of the agent adding the episode"
    )
    business_context: Optional[GraphitiBusinessContext] = Field(
        default=None,
        description="Business context for the episode"
    )
...
class GraphitiEpisodeResponse(BaseModel):
    ...
    agent_name: Optional[str] = Field(
        default=None,
        description="Agent that created the episode"
    )
...
class GraphitiSearchRequest(BaseModel):
    ...
    center_node_uuid: Optional[str] = Field(
        default=None,
        description="Optional center node for reranking results"
    )
    limit: int = Field(
        default=10,
        ge=1,
...
class GraphitiSearchResult(BaseModel):
    ...
    invalid_at: Optional[datetime] = Field(
        default=None,
        description="When this fact became invalid"
    )
...
class GraphitiAgentContext(BaseModel):
    ...
    agent_name: GraphitiAgentName = Field(
        ..., 
        description="Name of the agent"
    )
    business_context: Optional[GraphitiBusinessContext] = Field(
        default=None,
        description="Business context filter"
    )
    search_results: List[GraphitiSearchResult] = Field(
        default_factory=list,
        description="Recent search results for this agent"
    )
    recent_episodes: List[Dict[str, Any]] = Field(
        default_factory=list,
        description="Recent episodes created by this agent"
    )
    ...
    total_episodes: int = Field(
        default=0,
        description="Total number of episodes for this agent"
    )
...
class GraphitiHealthStatus(BaseModel):
    ...
    initialized_at: Optional[datetime] = Field(
        default=None,
        description="When the service was initialized"
    )
    cache_ttl: int = Field(
        default=300,
...
class GraphitiMemoryUpdate(BaseModel):
    ...
    agent_name: GraphitiAgentName = Field(
        ..., 
        description="Agent updating memory"
    )
    ...
    entities: List[str] = Field(
        default_factory=list,
        description="Entities mentioned in the interaction"
    )
    relationships: List[str] = Field(
        default_factory=list,
        description="Relationships identified"
    )
