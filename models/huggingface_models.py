...
class HuggingFaceModelConfig(BaseModel):
    ...
    task: str = Field(
        ..., description="Task type like 'text-generation', 'summarization', etc."
    )
...
class NotionHuggingFaceIntegration(BaseModel):
    ...
    output_property: str = Field(
        ..., description="Notion property to update with the result"
    )
    workflow_instance_id: Optional[str] = Field(
        None, description="ID of the workflow instance in Notion"
    )
