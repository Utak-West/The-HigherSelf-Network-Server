...
from models.pipit_models import PipitPaymentStatus, PipitFeatureType, PipitCurrency
...
class VideoFeature(BaseModel):
    ...
    duration_days: Optional[int] = Field(None, description="Duration in days if applicable")
...
class VideoTransaction(BaseModel):
    ...
    transaction_type: VideoTransactionType = Field(..., description="Type of transaction")
    transaction_status: VideoTransactionStatus = Field(..., description="Status of the transaction")
    payment_id: Optional[str] = Field(None, description="ID of the related payment")
    payment_status: Optional[PipitPaymentStatus] = Field(None, description="Status of the payment")
    amount: float = Field(..., description="Transaction amount")
    currency: PipitCurrency = Field(PipitCurrency.USD, description="Transaction currency")
...
class VideoTransactionResponse(BaseModel):
    ...
    transaction_id: Optional[str] = Field(None, description="ID of the created transaction")
    payment_url: Optional[HttpUrl] = Field(None, description="URL to complete the payment if required")
...
class VideoTransactionStatusResponse(BaseModel):
    ...
    transaction_status: VideoTransactionStatus = Field(..., description="Status of the transaction")
    payment_status: Optional[PipitPaymentStatus] = Field(None, description="Status of the payment if applicable")
    video_id: str = Field(..., description="ID of the related video content")
    features: List[VideoFeature] = Field(..., description="Features purchased")
    amount: float = Field(..., description="Transaction amount")
