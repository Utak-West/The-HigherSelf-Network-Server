...
from models.base import (
    EntityType, ApiPlatform, NotificationChannel, TaskStatus, ContentReviewStatus
)
...
class ContactProfile(BaseModel):
    ...
    """
    contact_id: str = Field(default_factory=lambda: f"CTCT-{uuid4().hex[:8]}", description="Unique contact ID")
    full_name: str = Field(..., description="Contact's full name")
    email: str = Field(..., description="Email address")
    phone: Optional[str] = Field(None, description="Phone number")
    contact_type: str = Field("Lead", description="Contact type (Lead, Client, Partner, etc.)")
    ...
    last_contacted_date: Optional[datetime] = Field(None, description="Last contacted date") 
    marketing_segments: List[str] = Field(default_factory=list, description="Marketing segments for targeting")
    ...
    engagement_score: Optional[int] = Field(None, description="Engagement score (0-100)")
...
class CommunityMember(BaseModel):
    ...
    """
    member_id: str = Field(default_factory=lambda: f"MEMBER-{uuid4().hex[:8]}", description="Unique member ID")
    ...
    interest_groups: List[str] = Field(default_factory=list, description="Interest groups joined")
    last_engagement_date: Optional[datetime] = Field(None, description="Last engagement date")
    engagement_type: Optional[str] = Field(None, description="Last engagement type (post, comment, etc.)")
    engagement_score: Optional[int] = Field(None, description="Engagement score (0-100)")
    events_attended: List[str] = Field(default_factory=list, description="List of attended events")
    ...
    related_contact: Optional[str] = Field(None, description="Related record in Contacts & Profiles DB")
...
class ProductService(BaseModel):
    ...
    """
    product_id: str = Field(default_factory=lambda: f"PROD-{uuid4().hex[:8]}", description="Unique product/service ID")
    name: str = Field(..., description="Product/service name")
    type: str = Field(..., description="Type (e.g., Course, Retreat, Art, Consultation)")
    description: str = Field(..., description="Detailed description")
    price: float = Field(..., description="Price in USD")
    status: str = Field("Active", description="Status (Active, Inactive, Coming Soon)")
    inventory_count: Optional[int] = Field(None, description="Inventory count for physical products")
    business_entity: str = Field(..., description="Related business entity ID")
    integration_system: Optional[str] = Field(None, description="Integration system (Amelia, WooCommerce, etc.)")
    external_id: Optional[str] = Field(None, description="ID in external system")
    images: List[str] = Field(default_factory=list, description="Image URLs")
    booking_link: Optional[str] = Field(None, description="Direct link to book/purchase")
    duration_minutes: Optional[int] = Field(None, description="Duration in minutes (for services)")
    categories: List[str] = Field(default_factory=list, description="Categories for filtering")
    tags: List[str] = Field(default_factory=list, description="Tags for filtering")
    featured: bool = Field(False, description="Whether this is a featured product/service")
...
class MarketingCampaign(BaseModel):
    ...
    """
    campaign_id: str = Field(default_factory=lambda: f"CAMP-{uuid4().hex[:8]}", description="Unique campaign ID")
    ...
    status: str = Field("Planning", description="Status (Planning, Active, Completed, Cancelled)")
    business_entity: str = Field(..., description="Related business entity ID")
    target_contacts: Dict[str, Any] = Field(..., description="Target segments definition")
    ...
    leads_generated: Optional[int] = Field(None, description="Number of leads generated")
    conversions: Optional[int] = Field(None, description="Number of conversions")
    related_notification_templates: List[str] = Field(default_factory=list, description="Related notification templates")
    results_summary: Optional[str] = Field(None, description="Summary of campaign results")
    url_parameters: Optional[str] = Field(None, description="UTM parameters for tracking")
...
class FeedbackSurvey(BaseModel):
    ...
    """
    feedback_id: str = Field(default_factory=lambda: f"FDBK-{uuid4().hex[:8]}", description="Unique feedback ID")
    ...
    related_product_service: Optional[str] = Field(None, description="Related product/service ID")
    related_workflow_instance: Optional[str] = Field(None, description="Related workflow instance ID")
    date_received: datetime = Field(default_factory=datetime.now, description="Date received")
    ...
    sentiment: Optional[str] = Field(None, description="Sentiment analysis (Positive, Neutral, Negative)")
    ...
    action_items: List[str] = Field(default_factory=list, description="Action items from feedback")
    tags: List[str] = Field(default_factory=list, description="Tags for categorization")
...
class RewardBounty(BaseModel):
    ...
    """
    reward_id: str = Field(default_factory=lambda: f"RWD-{uuid4().hex[:8]}", description="Unique reward ID")
    name: str = Field(..., description="Reward name")
    type: str = Field(..., description="Type (Referral, Commission, Loyalty Reward, etc.)")
    ...
    qualifying_action: str = Field(..., description="Action that qualifies for the reward")
    ...
    related_transaction: Optional[str] = Field(None, description="Related transaction ID")
    related_product_service: Optional[str] = Field(None, description="Related product/service ID")
