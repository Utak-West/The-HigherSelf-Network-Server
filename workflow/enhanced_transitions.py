...
class EnhancedStateMachine:
    ...
    async def enhanced_transition(
        ...
        try:
            ...
            # Find the transition
            transition = next((t for t in self.state_machine.transitions if t.name == transition_name), None)
            if not transition:
                result.error = f"Transition '{transition_name}' not found in workflow definition"
                self.logger.error(result.error)
                return result
            ...
            if hasattr(transition, 'evaluate_conditions'):
                conditions_met = await transition.evaluate_conditions(instance, condition_evaluator)
                if not conditions_met:
                    result.error = f"Transition conditions not met for '{transition_name}'"
                    self.logger.warning(result.error)
                    return result
            ...
            if hasattr(transition, 'get_dynamic_target_state'):
                ...
                if dynamic_target:
                    ...
                    self.logger.info(f"Dynamic routing: redirecting to '{target_state_name}' instead of '{transition.to_state}'")
            ...
            if hasattr(transition, 'precondition_actions'):
                for action_name in transition.precondition_actions:
                    try:
                        self.logger.info(f"Executing pre-transition action: {action_name}")
                        # Implement action handler mechanism
                    except Exception as e:
                        result.error = f"Error in pre-transition action '{action_name}': {str(e)}"
                        self.logger.error(result.error)
                        return result
            ...
            if hasattr(transition, 'postcondition_actions'):
                for action_name in transition.postcondition_actions:
                    try:
                        self.logger.info(f"Executing post-transition action: {action_name}")
                        # Implement action handler mechanism
                    except Exception as e:
                        # Note: we continue even if post-actions fail, but log the error
                        self.logger.error(f"Error in post-transition action '{action_name}': {str(e)}")
            ...
            if notion_success:
                ...
                self.logger.info(f"Transitioned instance {instance_id} from {current_state_name} to {target_state_name}")
            ...
            if retry_attempt < max_retries:
                ...
                self.logger.info(f"Recommending retry in {retry_delay} seconds (attempt {retry_attempt+1}/{max_retries})")
    ...
    async def transition_with_retry(
        ...
        while attempt < retry_count and result.retry_recommended:
            ...
            self.logger.info(f"Retrying transition '{transition_name}' (attempt {attempt+1}/{retry_count+1})")
            ...
            result = await self.enhanced_transition(
                ...
                transition_name=transition_name,
                agent_id=agent_id,
                action_description=f"{action_description} (retry {attempt})" if action_description else f"Retry {attempt}",
                transition_data=transition_data,
    ...
    async def assign_agent_to_state(
        ...
        context_assignments = [a for a in state.agent_assignments if a.context_dependent]
        ...
        semantic_assignments = [a for a in state.agent_assignments 
