...
def setup_notion_database(parent_page_id=None):
    ...
    if not parent_id:
        logger.error(
            "No parent page ID provided. Please set NOTION_PARENT_PAGE_ID in .env or provide it as an argument."
        )
        sys.exit(1)
    ...
    if not os.environ.get("NOTION_API_TOKEN"):
        logger.error(
            "NOTION_API_TOKEN is not set in the environment. Please set it in .env."
        )
        sys.exit(1)
    ...
    try:
        ...
        logger.info(
            "The database ID has been added to your .env file as NOTION_AGENT_TASKS_DB."
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Set up a Notion database for AI agent tasks."
    )
    parser.add_argument(
        "--parent-id",
        help="The ID of the parent page where the database will be created.",
    )
