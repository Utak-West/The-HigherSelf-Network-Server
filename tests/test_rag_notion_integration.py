...
async def test_notion_integration():
    ...
    if embedding_ids:
        print(f"✅ Successfully synced {len(embedding_ids)} pages from '{test_db_name}' database")
        return True
    else:
    ...
    if result.success:
        ...
        if result.sources:
            ...
            for i, source in enumerate(result.sources):
                print(f"  {i+1}. {source.title or source.source} (similarity: {source.similarity:.2f})")
