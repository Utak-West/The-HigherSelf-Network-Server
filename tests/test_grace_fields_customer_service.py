...
@pytest.fixture
def enhanced_grace(mock_agents, mock_notion_service, mock_escalation_service, mock_message_bus):
...
class TestEnhancedGraceFields:
    ...
    async def test_analyze_request_billing_issue(self, enhanced_grace):
        ...
        category, severity = await enhanced_grace._analyze_request(description, business_entity)
    ...
    async def test_standard_delegation(self, enhanced_grace):
        ...
        enhanced_grace.escalation_service.create_escalation_ticket = AsyncMock(return_value="ESC-12345")
    ...
    def test_get_primary_agent_for_category(self, enhanced_grace):
        ...
        assert enhanced_grace._get_primary_agent_for_category(IssueCategory.BILLING_ORDER) == "Solari"
        assert enhanced_grace._get_primary_agent_for_category(IssueCategory.CUSTOMER_FEEDBACK) == "Sage"
        assert enhanced_grace._get_primary_agent_for_category(IssueCategory.LEAD_MANAGEMENT) == "Nyra"
        assert enhanced_grace._get_primary_agent_for_category(IssueCategory.MARKETING_CAMPAIGN) == "Liora"

    def test_get_coordination_agents_for_category(self, enhanced_grace):
        """Test multi-agent coordination selection."""
        agents = enhanced_grace._get_coordination_agents_for_category(IssueCategory.VIP_SERVICE)
        assert len(agents) >= 3
    ...
    def test_agent_coordination_action_mapping(self, enhanced_grace):
        ...
        action = enhanced_grace._get_agent_coordination_action("Nyra", IssueCategory.BILLING_ORDER)
        assert "review customer history" in action
        
        action = enhanced_grace._get_agent_coordination_action("Solari", IssueCategory.VIP_SERVICE)
