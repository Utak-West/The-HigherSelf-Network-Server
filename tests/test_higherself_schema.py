...
try:
    ...
    print(
        f"Initial import failed: {e}. Attempting to adjust sys.path for testing context."
    )
...
def server_doc() -> HigherSelfNetworkServer:
    if not JSON_FILE_PATH.exists():
        pytest.fail(
            f"{JSON_FILE_PATH} not found. Make sure it's created in the root directory."
        )
    try:
...
def test_workflow_details(server_doc: HigherSelfNetworkServer):
    ...
    lead_workflow = next(
        (
            wf
            for wf in server_doc.workflows
            if wf.name == "Standard Lead Processing Workflow"
        ),
        None,
    )
