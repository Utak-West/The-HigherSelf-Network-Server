...
from .models import (HuggingFaceAgent, HuggingFaceDataset,
                     HuggingFaceModelReference, HuggingFaceSpace, ModelType,
                     SpaceFramework)
...
class HuggingFaceService:
    ...
    def _login(self) -> None:
        ...
        logger.info(
            f"Authenticated with Hugging Face API as organization: {self.config.organization or 'default'}"
        )
    ...
    def create_notion_model_reference(
        self,
        model_id: str,
        notion_id: str,
        ...
        if "pipeline_tag" in model_info:
            try:
                ...
                logger.warning(
                    f"Unknown pipeline tag: {model_info['pipeline_tag']}, defaulting to {model_type}"
                )
    ...
    def run_inference(
        ...
        **parameters,
        ...
        return self.inference_client.post(
            model_id, json={"inputs": inputs, "parameters": parameters}, task=task
        )
    ...
    def create_notion_space_reference(
        self,
        space_id: str,
        notion_id: str,
        name: Optional[str] = None,
        description: Optional[str] = None,
        framework: Optional[SpaceFramework] = None,
    ) -> HuggingFaceSpace:
        """
        ...
        if "/" not in space_id:
            if self.config.organization:
                ...
                raise ValueError(
                    f"Space ID {space_id} must include username/org name if no organization is configured"
                )
    ...
    def upload_to_space(
        self, space_id: str, file_path: str, path_in_repo: str
    ) -> Dict[str, Any]:
        """
