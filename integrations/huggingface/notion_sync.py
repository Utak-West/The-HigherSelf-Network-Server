...
class NotionHuggingFaceSync:
    ...
    def __init__(self, notion_client: Client, hf_service: HuggingFaceService, config: NotionHuggingFaceConfig):
        """
    ...
    def _add_to_history_log(self, notion_page_id: str, action: str, details: Dict[str, Any]):
        """
        ...
        try:
            ...
            current_log = page.get("properties", {}).get("History Log", {}).get("rich_text", [])
            ...
            logger.error(f"Failed to update history log for page {notion_page_id}: {str(e)}")
    ...
    def sync_model_to_notion(self, model: HuggingFaceModelReference) -> str:
        ...
        properties = {
            ...
            "Description": {"rich_text": [{"text": {"content": model.description or ""}}]},
            "Status": {"select": {"name": model.status}},
            "URL": {"url": str(model.hub_url)},
            "Tags": {"multi_select": [{"name": tag} for tag in model.tags[:10]]},  # Notion has a limit
        ...
        if model.notion_id:
            ...
            logger.info(f"Created new Notion page for model {model.model_id} with ID {notion_id}")
    ...
    def sync_space_to_notion(self, space: HuggingFaceSpace) -> str:
        ...
        properties = {
            ...
            "Description": {"rich_text": [{"text": {"content": space.description or ""}}]},
            "Status": {"select": {"name": space.status}},
            "URL": {"url": str(space.space_url)},
            "Hardware": {"select": {"name": space.hardware}},
            "Tags": {"multi_select": [{"name": tag} for tag in space.tags[:10]]},  # Notion has a limit
        ...
        if space.notion_id:
            ...
            notion_id = response["id"]
            logger.info(f"Created new Notion page for space {space.space_id} with ID {notion_id}")
            
            # Add to history log
            self._add_to_history_log(
                notion_id, 
                "Created Space Reference", 
                {"space_id": space.space_id}
            )
            return notion_id
    ...
    def sync_agent_to_notion(self, agent: HuggingFaceAgent) -> str:
        ...
        properties = {
            ...
            "Description": {"rich_text": [{"text": {"content": agent.description or ""}}]},
            "Status": {"select": {"name": agent.status}},
            "Tools": {"multi_select": [{"name": tool.tool_name[:100]} for tool in agent.tools[:10]]},  # Notion has limits
        ...
        if agent.system_prompt:
            content = [
                ...
                },
                {
                    ...
                    "type": "paragraph",
                    "paragraph": {
                        "rich_text": [{"type": "text", "text": {"content": agent.system_prompt}}]
                    }
                }
            ]
        ...
        if agent.notion_id:
            ...
            notion_id = response["id"]
            logger.info(f"Created new Notion page for agent {agent.name} with ID {notion_id}")
    ...
    def get_models_from_notion(self) -> List[HuggingFaceModelReference]:
        ...
        for page in response.get("results", []):
            try:
                ...
                notion_id = page["id"]
                name = "".join([text.get("text", {}).get("content", "") 
                               for text in properties.get("Name", {}).get("title", [])])
                
                model_id = "".join([text.get("text", {}).get("content", "") 
                                   for text in properties.get("Model ID", {}).get("rich_text", [])])
                
                description = "".join([text.get("text", {}).get("content", "") 
                                      for text in properties.get("Description", {}).get("rich_text", [])])
                
                model_type_name = properties.get("Type", {}).get("select", {}).get("name", "text-generation")
                status = properties.get("Status", {}).get("select", {}).get("name", "active")
                
                hub_url = properties.get("URL", {}).get("url", f"https://huggingface.co/{model_id}")
                ...
                logger.error(f"Error parsing model from Notion page {page.get('id')}: {str(e)}")
                continue
    ...
    def get_spaces_from_notion(self) -> List[HuggingFaceSpace]:
        ...
        for page in response.get("results", []):
            try:
                ...
                space_id = "".join([text.get("text", {}).get("content", "") 
                                   for text in properties.get("Space ID", {}).get("rich_text", [])])
                
                description = "".join([text.get("text", {}).get("content", "") 
                                      for text in properties.get("Description", {}).get("rich_text", [])])
                
                framework_name = properties.get("Framework", {}).get("select", {}).get("name", "gradio")
                status = properties.get("Status", {}).get("select", {}).get("name", "active")
                hardware = properties.get("Hardware", {}).get("select", {}).get("name", "cpu-basic")
                
                space_url = properties.get("URL", {}).get("url", f"https://huggingface.co/spaces/{space_id}")
                ...
                logger.error(f"Error parsing space from Notion page {page.get('id')}: {str(e)}")
                continue
    ...
    def get_agents_from_notion(self) -> List[HuggingFaceAgent]:
        ...
        for page in response.get("results", []):
            try:
                ...
                base_model_id = "".join([text.get("text", {}).get("content", "") 
                                      for text in properties.get("Base Model", {}).get("rich_text", [])])
                ...
                try:
                    ...
                    logger.warning(f"Error retrieving system prompt for agent {name}: {str(e)}")
                ...
                logger.error(f"Error parsing agent from Notion page {page.get('id')}: {str(e)}")
                continue
        ...
        try:
            ...
            for model in notion_models:
                try:
                    ...
                    if model.name != model_info.get("modelId") or model.description != model_info.get("description"):
            ...
            for space in notion_spaces:
                try:
                    ...
                    if space.name != space_info.get("id") or space.description != space_info.get("description"):
            ...
            for agent in notion_agents:
                try:
                    ...
                    logger.error(f"Error validating agent base model {agent.base_model_id}: {str(e)}")
                    self._add_to_history_log(
                        agent.notion_id,
                        "Validation Error",
