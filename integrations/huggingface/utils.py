...
def load_huggingface_config_from_env() -> HuggingFaceIntegrationConfig:
    ...
    if not api_key:
        raise ValueError(
            "HF_API_KEY environment variable is required for Hugging Face integration"
        )
...
def load_notion_huggingface_config_from_env() -> NotionHuggingFaceConfig:
    ...
    if not database_id:
        raise ValueError(
            "NOTION_HF_DATABASE_ID environment variable is required for Hugging Face-Notion integration"
        )
    ...
    history_log = os.environ.get("NOTION_HF_HISTORY_LOG", "true").lower() in (
        "true",
        "1",
        "yes",
    )
