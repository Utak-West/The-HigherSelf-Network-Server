...
from ..models.softr_integration_models import (ApiResponse,
...
class HigherSelfNetworkAPI:
    ...
    def _test_connection(self) -> bool:
        ...
        if response.status_code != 200:
            raise ConnectionError(
                f"API connection failed with status {response.status_code}"
            )
        return True
    ...
    async def trigger_workflow(self, request: WorkflowTriggerRequest) -> Dict[str, Any]:
        ...
        try:
            response = self.session.post(
                f"{self.base_url}/workflows/trigger", json=request.dict()
            )
            response.raise_for_status()
    ...
    async def get_data_collection(
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/collections/{collection_name}", params=params
            )
            response.raise_for_status()
            return response.json()
        except Exception as e:
    ...
    async def create_collection_item(
        self, collection_name: str, item_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            response = self.session.post(
                f"{self.base_url}/collections/{collection_name}", json=item_data
            )
            response.raise_for_status()
            return response.json()
        except Exception as e:
    ...
    async def update_collection_item(
        self, collection_name: str, item_id: str, item_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    async def create_purchase(self, purchase_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            response = self.session.post(
                f"{self.base_url}/purchases", json=purchase_data
            )
            response.raise_for_status()
            return response.json()
        except Exception as e:
...
def create_api_client(
    config: Optional[SoftrIntegrationConfig] = None,
) -> HigherSelfNetworkAPI:
    """
