...
from .config import MCPToolConfig, mcp_config
...
class MCPToolRegistry:
    ...
    def _register_built_in_tools(self):
        ...
        # Perplexity tool
        self.register_tool_handler(
            "perplexity", self._handle_perplexity_tool, ["ask", "web_search"]
        )
    ...
    def register_tool_handler(
        self, tool_name: str, handler: Callable, operations: List[str] = None
    ):
        """Register a new MCP tool handler."""
        config = mcp_config.get_config(tool_name)
        if not config or not config.enabled:
            logger.warning(
                f"Cannot register handler for disabled or missing tool: {tool_name}"
            )
            return False
        ...
        if config.rate_limit_per_min:
            self._rate_limiters[tool_name] = MCPRateLimiter(
                tool_name, config.rate_limit_per_min
            )

        logger.info(
            f"Registered MCP tool handler for {tool_name} with operations: {operations}"
        )
        return True

    async def execute_tool(
        self, agent: Agent, tool_name: str, operation: str, params: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Execute an MCP tool."""
    ...
    async def _handle_context7_tool(
    ...
    async def _handle_perplexity_tool(
        self, agent: Agent, operation: str, params: Dict[str, Any]
    ) -> Dict[str, Any]:
