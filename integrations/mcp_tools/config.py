...
class MCPToolConfig(BaseModel):
    ...
    max_retries: int = 3
    requires_permission: bool = False  # Whether agent needs explicit permission to use this tool
    allowed_agents: Set[str] = Field(default_factory=lambda: {"*"})  # * means all agents
    ...
    def _initialize(self):
        ...
        self.configs = {
            ...
            "context7": MCPToolConfig(
                server_url=os.environ.get("MCP_CONTEXT7_URL", "https://api.context7.com/v1"),
                api_key=os.environ.get("MCP_CONTEXT7_API_KEY"),
                auth_type="api_key",
                timeout=45,  # Longer timeout for RAG operations
                requires_permission=False,
                allowed_agents=set(os.environ.get("MCP_CONTEXT7_ALLOWED_AGENTS", "*").split(",")),
            ),
            # Memory tool for knowledge persistence
            "memory": MCPToolConfig(
                server_url=os.environ.get("MCP_MEMORY_URL", "https://api.memory.dev/v1"),
                api_key=os.environ.get("MCP_MEMORY_API_KEY"),
                auth_type="api_key",
                requires_permission=False,
                allowed_agents=set(os.environ.get("MCP_MEMORY_ALLOWED_AGENTS", "*").split(",")),
            ),
            # Perplexity for advanced search capabilities
            "perplexity": MCPToolConfig(
                server_url=os.environ.get("MCP_PERPLEXITY_URL", "https://api.perplexity.ai"),
                ...
                allowed_agents=set(os.environ.get("MCP_PERPLEXITY_ALLOWED_AGENTS", "research,knowledge").split(",")),
                rate_limit_per_min=10
            ),
            # Brave search for web search capabilities
            "brave_search": MCPToolConfig(
                server_url=os.environ.get("MCP_BRAVE_SEARCH_URL", "https://api.search.brave.com"),
                api_key=os.environ.get("MCP_BRAVE_SEARCH_API_KEY"),
                auth_type="api_key",
                requires_permission=True,
                allowed_agents=set(os.environ.get("MCP_BRAVE_SEARCH_ALLOWED_AGENTS", "research").split(",")),
                rate_limit_per_min=30
            ),
            # Sequential thinking for complex reasoning tasks
            "sequential_thinking": MCPToolConfig(
                server_url=os.environ.get("MCP_SEQ_THINKING_URL", "https://api.sequential-thinking.com"),
                api_key=os.environ.get("MCP_SEQ_THINKING_API_KEY"),
                auth_type="api_key",
                requires_permission=False,
                allowed_agents=set(os.environ.get("MCP_SEQ_THINKING_ALLOWED_AGENTS", "*").split(",")),
            )
        }
    ...
    def _load_custom_tools(self):
        ...
        for env_name, env_value in os.environ.items():
            if env_name.startswith("MCP_TOOL_") and "_URL" in env_name:
                tool_name = env_name.replace("MCP_TOOL_", "").replace("_URL", "").lower()
                ...
                custom_tools[tool_name] = MCPToolConfig(
                    ...
                    requires_permission=os.environ.get(requires_perm_env, "false").lower() == "true",
                    allowed_agents=set(os.environ.get(allowed_agents_env, "*").split(",")),
                    rate_limit_per_min=int(os.environ.get(rate_limit_env)) if os.environ.get(rate_limit_env) else None
                )
        ...
        for name, config in self.configs.items():
            if config.enabled:
                ...
                if config.auth_type == "api_key" and not config.api_key:
                    logger.warning(f"MCP tool '{name}' is configured with auth_type='api_key' but no API key is provided. Tool will be disabled.")
                    config.enabled = False
                
                # Log validation status
                if config.enabled:
                    logger.info(f"MCP tool '{name}' configuration validated and enabled.")
                    if config.requires_permission:
                        logger.info(f"MCP tool '{name}' requires explicit permission for use.")
                else:
                    logger.warning(f"MCP tool '{name}' is disabled due to invalid configuration.")
    ...
    def get_tools_for_agent(self, agent_id: str) -> Dict[str, MCPToolConfig]:
        ...
        return {
            ...
            if config.enabled and (
                "*" in config.allowed_agents or 
                agent_id in config.allowed_agents
            )
        }
