...
class WebBrowserTool:
        ...
        metadata = ToolMetadata(
            ...
            parameters_schema={
                ...
                "properties": {
                    "operation": {
                        "type": "string",
                        "enum": ["search", "get_page", "extract_links", "summarize"],
            ...
            examples=[
                ...
                {"operation": "extract_links", "url": "https://example.com"},
            ],
        )
    ...
    async def handle_web_operation(
        ...
        try:
            ...
            return {"success": False, "message": f"Error in web operation: {str(e)}"}
    ...
    async def _google_search(
        self, query: str, num_results: int, agent_id: str
    ) -> Dict[str, Any]:
    ...
    async def _basic_search(
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get(
                search_url, headers=headers, follow_redirects=True, timeout=10
            )
            ...
            async with httpx.AsyncClient() as client:
                ...
                if select_element:
                    ...
                    if selected:
                        ...
                        content_text = " ".join(
                            e.get_text(" ", strip=True) for e in selected
                        )
                    else:
                        return {
                            "success": False,
            ...
            async with httpx.AsyncClient() as client:
                ...
                for a in soup.find_all("a", href=True):
                    ...
                    if href.startswith(("http://", "https://")):
                        links.append(
                            {"url": href, "text": link_text if link_text else href}
                        )
    ...
    async def _summarize_page(
        ...
        try:
            ...
            page_result = await self._get_page(
                {"url": url, "extract_text_only": True}, agent_id
            )
            ...
            if len(sentences) > 10:
                summary_sentences = (
                    sentences[:3]
                    + sentences[len(sentences) // 2 - 1 : len(sentences) // 2 + 1]
                    + sentences[-3:]
                )
            else:
