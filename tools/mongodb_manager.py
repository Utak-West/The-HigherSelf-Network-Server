...
class MongoDBManager:
    ...
    def backup_database(self, backup_path: str = None) -> bool:
        ...
        try:
            ...
            for collection_name in collections:
                ...
                print_info(f"Backed up collection '{collection_name}': {len(documents)} documents")
        ...
        try:
            ...
            for collection_name, documents in backup_data.items():
                ...
                if documents:
                    ...
                    print_info(f"Restored collection '{collection_name}': {len(documents)} documents")
    ...
    def print_collection_stats(self):
        ...
        for collection_name, collection_stats in stats.items():
            ...
            print(f"{collection_name:<25} {collection_stats['count']:<12} {size_mb:<12.2f} {collection_stats['indexes']:<8}")
    ...
    def print_performance_stats(self):
        ...
        print(f"Connections - Current: {connections.get('current', 0)}, Available: {connections.get('available', 0)}")
        
        # Operations
        opcounters = perf_data.get('opcounters', {})
        print(f"Operations - Insert: {opcounters.get('insert', 0)}, Query: {opcounters.get('query', 0)}, Update: {opcounters.get('update', 0)}")
        
        # Memory
        memory = perf_data.get('memory', {})
        print(f"Memory - Resident: {memory.get('resident', 0)} MB, Virtual: {memory.get('virtual', 0)} MB")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="MongoDB Management for HigherSelf Network")
    parser.add_argument('command', choices=[
        'backup', 'restore', 'cleanup', 'optimize', 'stats', 'performance'
