...
class AgentMessageInput(BaseModel):
    target_agent: str = Field(description="Target agent name (Nyra, Solari, Ruvo, Liora, Sage, Elan, Zevi, Grace, Atlas)")
...
class AgentCommunicationTool(BaseTool):
    ...
    def _run(self, target_agent: str, message: str, priority: str = "normal", context: Dict[str, Any] = None) -> str:
        ...
        try:
            ...
            logger.info(f"Message sent from {self.sender_agent} to {target_agent} (priority: {priority})")
            return f"Message sent to {target_agent} successfully"
            
        except Exception as e:
            logger.error(f"Error sending message from {self.sender_agent} to {target_agent}: {e}")
            return f"Error sending message: {str(e)}"
    
    async def _arun(self, target_agent: str, message: str, priority: str = "normal", context: Dict[str, Any] = None) -> str:
        """Async version."""
...
class WorkflowTriggerInput(BaseModel):
    ...
    scheduled_time: str = Field(default="", description="Optional scheduled execution time (ISO format)")
...
class WorkflowTriggerTool(BaseTool):
    ...
    def _run(self, workflow_name: str, context: Dict[str, Any], priority: str = "normal", scheduled_time: str = "") -> str:
        """Trigger a workflow."""
        try:
            workflow_data = {
            ...
            logger.info(f"Workflow '{workflow_name}' triggered by {self.triggering_agent} (priority: {priority})")
    ...
    async def _arun(self, workflow_name: str, context: Dict[str, Any], priority: str = "normal", scheduled_time: str = "") -> str:
        """Async version."""
...
class TaskCreationInput(BaseModel):
    ...
    due_date: str = Field(default="", description="Due date (ISO format or relative like 'tomorrow', 'next week')")
...
class TaskCreationTool(BaseTool):
    ...
    def _run(self, title: str, description: str, assignee: str, priority: str = "medium", due_date: str = "", context: Dict[str, Any] = None) -> str:
        """Create a new task."""
        try:
            if context is None:
                context = {}
            ...
            if assignee in ["Nyra", "Solari", "Ruvo", "Liora", "Sage", "Elan", "Zevi", "Grace", "Atlas"]:
                notification_data = {
                    "from": self.creating_agent,
                    "to": assignee,
                    "message": f"New task assigned: {title}",
                    "priority": priority,
                    "context": {"task_id": task_data["task_id"], "task_data": task_data},
                    "timestamp": datetime.utcnow().isoformat()
                }
            ...
            logger.info(f"Task '{title}' created by {self.creating_agent} and assigned to {assignee}")
    ...
    async def _arun(self, title: str, description: str, assignee: str, priority: str = "medium", due_date: str = "", context: Dict[str, Any] = None) -> str:
        """Async version."""
...
class NotificationInput(BaseModel):
    ...
    notification_type: str = Field(default="info", description="Notification type: info, warning, error, success")
    channels: list = Field(default_factory=lambda: ["redis"], description="Notification channels: redis, email, slack")
...
class NotificationTool(BaseTool):
    ...
    def _run(self, recipient: str, message: str, notification_type: str = "info", channels: list = None) -> str:
        """Send a notification."""
        try:
            if channels is None:
                channels = ["redis"]
            ...
            if "redis" in channels:
                if recipient == "all":
                    ...
                    redis_service.publish("higherself:notifications:broadcast", json.dumps(notification_data))
                else:
            ...
            logger.info(f"Notification sent from {self.sending_agent} to {recipient} (type: {notification_type})")
    ...
    async def _arun(self, recipient: str, message: str, notification_type: str = "info", channels: list = None) -> str:
        """Async version."""
