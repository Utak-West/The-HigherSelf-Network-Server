...
class LeadQualificationTool(BaseTool):
    ...
    def _run(self, lead_data: str) -> str:
        ...
        try:
            ...
            if data.get("company") or any(
                word in message for word in ["company", "organization", "team", "group"]
            ):
                score += 1
            ...
            if not recommendations:
                if score >= 6:
                    ...
                    recommendations.append(
                        "Medium priority - follow-up within 24 hours"
                    )
                else:
...
class ContentAnalysisInput(BaseModel):
    ...
    analysis_type: str = Field(
        default="general",
        description="Type of analysis: sentiment, intent, topic, quality",
    )
...
class ContentAnalysisTool(BaseTool):
    ...
    def _assess_quality(self, content: str) -> Dict[str, Any]:
        ...
        quality_level = (
            "High" if quality_score >= 4 else "Medium" if quality_score >= 2 else "Low"
        )
