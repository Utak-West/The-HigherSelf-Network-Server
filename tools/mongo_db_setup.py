...
class MongoDBSetup:
    ...
    def create_index(
        self, collection_name: str, field_name: str, unique: bool = False
    ) -> bool:
        """Create an index on a collection field."""
        try:
            self.db[collection_name].create_index(
                [(field_name, ASCENDING)], unique=unique
            )
            print_info(
                f"Created index on '{collection_name}.{field_name}' (unique={unique})"
            )
            return True
        except Exception as e:
            print_error(
                f"Failed to create index on '{collection_name}.{field_name}': {e}"
            )
            return False
...
async def main():
    ...
    if all_success:
        ...
        print_warning(
            "Some collections or indexes could not be created. Check the logs for details."
        )
