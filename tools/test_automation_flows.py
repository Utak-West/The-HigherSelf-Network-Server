...
from config.testing_mode import enable_testing_mode, disable_testing_mode, is_testing_mode
...
class AutomationFlowTester:
    ...
    def __init__(self, use_test_mode: bool = True):
        ...
        if use_test_mode:
            ...
            print(f"{Fore.RED}WARNING: Running with real API calls. Data will be created in Notion.{Style.RESET_ALL}")
    ...
    async def initialize(self):
        ...
        if not os.environ.get("NOTION_API_TOKEN"):
            print(f"{Fore.RED}Error: NOTION_API_TOKEN not found in environment variables{Style.RESET_ALL}")
                ...
                "webhook": {
                    ...
                    "data": {
                        ...
                        "answers": [
                            ...
                            {"field": {"id": "interest"}, "choices": {"labels": ["Retreats", "Courses"]}}
                        ]
                    }
                }
            }
                ...
                "webhook": {
                    ...
                    "data": {
                        ...
                        "order": {
                            ...
                            "shipping": {
                                "address": "123 Art St, Creativity, CA 90210"
                            }
                        }
                    }
                }
    ...
    async def _process_flow(self, flow: AutomationFlow, mock_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if flow == AutomationFlow.LEAD_CAPTURE:
                return await self._simulate_webhook("webhooks/typeform", mock_data["webhook"])
            
            elif flow == AutomationFlow.RETREAT_BOOKING:
                return await self._simulate_webhook("webhooks/amelia", mock_data["webhook"])
            
            elif flow == AutomationFlow.ART_SALE:
                return await self._simulate_webhook("webhooks/woocommerce", mock_data["webhook"])
                ...
                return await self._simulate_webhook("webhooks/circleso/new_member", mock_data["webhook"])
            ...
            # Default case
            return {"success": False, "details": {"error": "Unknown flow", "flow": flow.value}}
            
        except Exception as e:
            print(f"{Fore.RED}Error processing flow: {str(e)}{Style.RESET_ALL}")
            return {"success": False, "details": {"error": str(e)}}
    
    async def _simulate_webhook(self, endpoint: str, payload: Dict[str, Any]) -> Dict[str, Any]:
...
async def main():
    ...
    parser.add_argument(
        "--real",
        action="store_true",
        help="Use real API calls instead of test mode"
    )
