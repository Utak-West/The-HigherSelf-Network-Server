...
async def check_services_health() -> bool:
    ...
    if knowledge_health["healthy"]:
        ...
        print_error(
            f"✗ Knowledge service is unhealthy: {knowledge_health.get('error', 'Unknown error')}"
        )
        return False
    ...
    if vector_health["healthy"]:
        ...
        print_error(
            f"✗ Vector store is unhealthy: {vector_health.get('error', 'Unknown error')}"
        )
        return False
    ...
    if provider_result["success"]:
        print_success(
            f"✓ Embedding provider ({provider_result['provider']}) is healthy"
        )
    else:
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Notion to Vector Store Synchronization Utility"
    )
    parser.add_argument(
        "--databases",
        nargs="+",
    ...
    if args.since:
        if args.since.endswith("d"):
            ...
            try:
                ...
                print_error(
                    f"Invalid time format: {args.since}. Use ISO format or relative time like '1d', '12h'"
                )
                sys.exit(1)
