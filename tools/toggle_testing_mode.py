...
def main():
    ...
    group.add_argument(
        "--enable", 
        action="store_true", 
        help="Enable testing mode (block API calls)"
    )
    group.add_argument(
        "--disable", 
        action="store_true", 
        help="Disable testing mode (allow API calls)"
    )
    group.add_argument(
        "--status", 
        action="store_true", 
        help="Check current testing mode status"
    )
    ...
    if args.status:
        if is_testing_mode():
            print(f"{Fore.YELLOW}Testing mode is currently {Fore.GREEN}ENABLED{Fore.YELLOW}.{Style.RESET_ALL}")
            print("External API calls are being blocked.")
        else:
            print(f"{Fore.YELLOW}Testing mode is currently {Fore.RED}DISABLED{Fore.YELLOW}.{Style.RESET_ALL}")
    ...
    if args.enable:
        ...
        print(f"{Fore.GREEN}Testing mode enabled. API calls will be blocked.{Style.RESET_ALL}")
        if api_list:
        ...
        print(f"{Fore.GREEN}Testing mode disabled. API calls will be made normally.{Style.RESET_ALL}")
