...
class NotionStructureValidator:
    ...
    async def validate_all_databases(self) -> Dict[str, Dict[str, Any]]:
        ...
        if missing_required:
            print(f"{Fore.RED}❌ Missing required environment variables: {', '.join(missing_required)}{Style.RESET_ALL}")
            all_valid = False
        ...
        for model_name, db_info in NOTION_DATABASE_MAPPING.items():
            ...
            if not database_id:
                status = {
                    ...
                    "is_valid": not db_info["required"]  # Only mark as invalid if required
                }
            else:
    ...
    def _check_environment_variables(self) -> Dict[str, List[str]]:
        ...
        missing_optional = [
            var for var in optional_env_vars 
            if not os.environ.get(var)
        ]
    ...
    async def _validate_database(self, model_name: str, database_id: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            status = {
                ...
                "message": "Database is accessible and structure appears valid" if is_valid_structure 
    ...
    def print_validation_results(self):
        ...
        if not self.database_statuses:
            print(f"{Fore.YELLOW}No validation results available. Run validate_all_databases() first.{Style.RESET_ALL}")
            return
        
        print("\n" + "="*80)
        print(f"{Fore.CYAN}NOTION DATABASE STRUCTURE VALIDATION RESULTS{Style.RESET_ALL}")
        print("="*80 + "\n")
        ...
        for model_name, status in self.database_statuses.items():
            ...
            # Print status line
            print(f"{status_indicator} {Fore.WHITE}{model_name}{Style.RESET_ALL} ({db_description})")
            print(f"   {message}")
        ...
        if all_valid:
            print(f"{Fore.GREEN}All required databases are properly configured and accessible.{Style.RESET_ALL}")
            print(f"{Fore.GREEN}The 16-database Notion structure is VALID for The HigherSelf Network Server.{Style.RESET_ALL}")
        else:
            print(f"{Fore.RED}Some required databases have issues. See above for details.{Style.RESET_ALL}")
            print(f"{Fore.RED}The Notion structure is INVALID. Please fix the issues before proceeding.{Style.RESET_ALL}")
