...
class MongoDBReviewer:
    ...
    def __init__(self):
        ...
        self.expected_collections = [
            'agents', 'workflows', 'workflow_instances', 'tasks',
            'agent_communication', 'agent_communication_registry',
            'api_integrations', 'system_health', 'data_transformations',
            'notification_templates', 'use_cases', 'business_entities',
            'contacts_profiles', 'community_hub', 'products_services',
            'marketing_campaigns', 'feedback_surveys', 'rewards_bounties'
        ]
    ...
    def connect(self) -> bool:
        ...
        for uri in connection_attempts:
            try:
                print_info(f"Attempting connection to: {uri.split('@')[-1] if '@' in uri else uri}")
    ...
    def review_connection_details(self):
        ...
        try:
            ...
            print_data("Connection URI", self.mongo_uri.split('@')[-1] if '@' in self.mongo_uri else self.mongo_uri)
            print_data("Database Name", self.db_name)
            
            # Connection status
            is_master = self.db.command('isMaster')
            print_data("Server Type", "Primary" if is_master.get('ismaster') else "Secondary")
        ...
        try:
            ...
            for collection_name in sorted(existing_collections):
                try:
                    ...
                    status = "✓ Expected" if collection_name in self.expected_collections else "⚠ Additional"
            ...
            missing_collections = set(self.expected_collections) - set(existing_collections)
    ...
    def review_indexes(self):
        ...
        key_collections = ['agents', 'workflows', 'tasks', 'system_health', 'agent_communication']
        
        try:
            for collection_name in key_collections:
                if collection_name not in self.db.list_collection_names():
                    print_warning(f"Collection '{collection_name}' not found, skipping index check")
                    continue
    ...
    def review_database_stats(self):
        ...
        try:
            ...
            print_data("  Data Size", f"{db_stats.get('dataSize', 0) / 1024 / 1024:.2f} MB")
            print_data("  Storage Size", f"{db_stats.get('storageSize', 0) / 1024 / 1024:.2f} MB")
            print_data("  Index Size", f"{db_stats.get('indexSize', 0) / 1024 / 1024:.2f} MB")
            ...
            try:
                ...
                print_data("  Available Connections", connections.get('available', 'N/A'))
                ...
                print_warning("Server status not accessible (may require admin privileges)")
        ...
        try:
            ...
            try:
                ...
                if authenticated_user_roles:
                    print_data("User Roles", ", ".join([role.get('role', 'Unknown') for role in authenticated_user_roles]))
    ...
    def review_initial_data(self):
        ...
        try:
            ...
            if 'system_health' in self.db.list_collection_names():
                ...
                if health_count > 0:
                    latest_health = self.db['system_health'].find_one(sort=[('timestamp', -1)])
                    if latest_health:
                        print_data("Latest Health Check", latest_health.get('timestamp', 'Unknown'))
    ...
    def review_higherself_alignment(self):
        ...
        requirements_check = {
            ...
            "Workflow Execution": "workflows" in self.db.list_collection_names() and "workflow_instances" in self.db.list_collection_names(),
            "Task Management": "tasks" in self.db.list_collection_names(),
            "Agent Communication": "agent_communication" in self.db.list_collection_names(),
            "API Integration Management": "api_integrations" in self.db.list_collection_names(),
            "System Health Monitoring": "system_health" in self.db.list_collection_names(),
            "Business Entity Registry": "business_entities" in self.db.list_collection_names(),
            "Community Hub": "community_hub" in self.db.list_collection_names(),
            "Marketing Campaigns": "marketing_campaigns" in self.db.list_collection_names()
        }
