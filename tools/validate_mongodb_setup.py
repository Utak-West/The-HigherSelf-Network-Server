...
class MongoDBValidator:
        ...
        try:
            ...
            for collection_name, expected_indexes in self.expected_indexes.items():
                if collection_name not in self.db.list_collection_names():
                    print_warning(f"Collection '{collection_name}' not found, skipping index validation")
                    continue
    ...
    async def test_async_operations(self) -> bool:
        ...
        try:
            ...
            # Find
            found_doc = await test_collection.find_one({'test_id': 'async_validation_test'})
            if found_doc:
...
async def main():
    ...
    try:
        ...
        if all([collections_valid, indexes_valid, basic_ops_valid, async_ops_valid]):
            ...
            print_info("Your MongoDB configuration is ready for HigherSelf Network operations")
        else:
            print_error("❌ MongoDB setup validation FAILED")
            print_info("Please review the errors above and run the setup script if needed")
            sys.exit(1)
