    ...
    def _format_results(self, results: Dict[str, Any], limit: int) -> str:
        ...
        for item in results["results"][:limit]:
            ...
            if properties_summary:
                item_summary += (
                    f" ({', '.join(properties_summary[:3])})"  # Limit to 3 properties
                )
...
class NotionCreatePageTool(BaseTool):
    ...
    async def _arun(
        self, database_id: str, title: str, properties: Dict[str, Any]
    ) -> str:
        """Async version."""
...
class NotionSearchInput(BaseModel):
    ...
    filter_type: str = Field(
        default="page", description="Type of content to search: page or database"
    )
