...
class AgentTrainingPipeline:
    ...
    async def train_agent(self, config: DatasetTrainingConfig) -> Optional[DatasetTrainingResult]:
        """
        ...
        try:
            ...
            if config.version_id:
                version = await self.version_repo.async_find_by_version_id(config.version_id)
            else:
                version = await self.version_repo.async_get_latest_version(config.dataset_id)
            ...
            if not version:
                ...
                training_result.error_message = f"Failed to get or create version for dataset {config.dataset_id}"
            ...
            X_train, X_test, y_train, y_test = await self.dataset_processor.prepare_training_data(config)
            
            if X_train is None:
                # Update training result with error
                training_result.status = "failed"
                training_result.error_message = f"Failed to prepare training data for dataset {config.dataset_id}"
                training_result.completed_at = datetime.now()
                await self.training_repo.async_save(training_result)
                return training_result
            ...
            if model is None:
                # Update training result with error
                training_result.status = "failed"
                training_result.error_message = f"Failed to train model for agent {config.agent_id}"
                training_result.completed_at = datetime.now()
                await self.training_repo.async_save(training_result)
                return training_result
    ...
    async def _train_model(
        ...
        try:
            ...
            is_classification = unique_values < 10  # Heuristic: if fewer than 10 unique values, treat as classification
            ...
            if model_type == "random_forest":
                if is_classification:
                    model = RandomForestClassifier(
                        n_estimators=config.training_parameters.get("n_estimators", 100),
                ...
                if is_classification:
                    model = MLPClassifier(
                        hidden_layer_sizes=config.training_parameters.get("hidden_layer_sizes", (100,)),
    ...
    async def _notify_agent(self, agent_id: str, training_result: DatasetTrainingResult) -> None:
        """
        ...
        try:
            ...
            message = AgentMessage(
                ...
                payload={
                    ...
                    "completed_at": training_result.completed_at.isoformat() if training_result.completed_at else None
                }
            )
    ...
    async def get_training_result(self, training_id: str) -> Optional[DatasetTrainingResult]:
        """
    ...
    async def load_trained_model(self, training_id: str) -> Optional[Any]:
        ...
        try:
            # Get training result
            training_result = await self.training_repo.async_find_by_training_id(training_id)
