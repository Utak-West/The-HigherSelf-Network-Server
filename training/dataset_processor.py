...
class DatasetProcessor:
    ...
    async def import_dataset(self, dataset_id: str) -> Optional[DatasetMetadata]:
        ...
        try:
            ...
            existing_dataset = await self.metadata_repo.async_find_by_openml_id(dataset_id)
            if existing_dataset:
                ...
                existing_dataset.default_target_attribute = dataset_meta.default_target_attribute
    ...
    async def prepare_training_data(
        ...
        try:
            ...
            if not target_column:
                logger.error(f"No target column specified for dataset {config.dataset_id}")
                return None, None, None, None
        ...
        for metric in metrics:
            try:
                if metric == "accuracy":
                    ...
                    results[metric] = float(precision_score(y_true, y_pred, average="weighted"))
                elif metric == "recall":
                    results[metric] = float(recall_score(y_true, y_pred, average="weighted"))
                elif metric == "f1":
                    results[metric] = float(f1_score(y_true, y_pred, average="weighted"))
                elif metric == "mse":
