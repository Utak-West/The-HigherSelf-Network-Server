...
class AgentGitResolver:
    ...
    def analyze_git_status(self) -> Dict:
        ...
        analysis = {
            ...
            "modified_files": [
                f[3:] for f in files if f.startswith(" M") or f.startswith("M ")
            ],
    ...
    def resolve_conflicts_by_category(self, categorized_conflicts: Dict) -> Dict:
        ...
        for category in high_priority:
            if (
                category in categorized_conflicts["categorized"]
                and categorized_conflicts["categorized"][category]
            ):
                files = categorized_conflicts["categorized"][category]
                strategy = categories[category]["strategy"]

                print(
                    f"   🔧 Resolving {category} ({len(files)} files) with strategy: {strategy}"
                )

                if strategy == "preserve_pr4_changes":
                    for file in files:
                        success, _, stderr = self._run_command(
                            f"git checkout --theirs {file}"
                        )
                        if success:
    ...
    def execute_agent_assisted_resolution(self) -> Dict:
        ...
        try:
            ...
            if results["merge"].get("conflicts"):
                results["categorization"] = self.categorize_conflicts(
                    results["merge"]["conflicts"]
                )
                results["resolution"] = self.resolve_conflicts_by_category(
