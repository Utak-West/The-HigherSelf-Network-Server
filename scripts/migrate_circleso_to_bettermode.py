...
class CirclesoToBettermodeConverter:
    ...
    async def get_circle_spaces(self) -> List[Dict[str, Any]]:
        ...
        params = {"community_id": self.circle_community_id}
    ...
    def convert_member_to_bettermode(
        self, circle_member: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def convert_space_to_bettermode(
        self, circle_space: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
...
async def migrate_members(
    ...
    while True:
        # Get members from Circle.so
        circle_members = await converter.get_circle_members(
        ...
        for circle_member in tqdm(circle_members, desc="Migrating members"):
            # Convert Circle.so member to BetterMode format
            bettermode_member_data = converter.convert_member_to_bettermode(
                circle_member
            )

            # Check if member already exists in BetterMode
            existing_member = await bettermode_service.get_member_by_email(
                bettermode_member_data["email"]
            )

            if existing_member:
                logger.info(
                    f"Member {bettermode_member_data['email']} already exists in BetterMode"
                )
                continue
            ...
            try:
                ...
                logger.info(
                    f"Creating member in BetterMode: {bettermode_member_data['email']}"
                )
                ...
                filter_conditions = {
                    ...
                    "rich_text": {"equals": bettermode_member_data["email"]},
                }
                ...
                if members:
                    ...
                    member.bettermode_member_id = (
                        "placeholder_id"  # Would be actual ID from BetterMode
                    )
                    ...
                    new_member = CommunityMember(
                        ...
                        membership_level=bettermode_member_data["custom_fields"][
                            "membership_level"
                        ],
                ...
                if max_members and total_migrated >= max_members:
                    logger.info(
                        f"Reached maximum number of members to migrate: {max_members}"
                    )
                    return total_migrated

            except Exception as e:
                logger.error(
                    f"Error migrating member {bettermode_member_data['email']}: {e}"
                )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Migrate data from Circle.so to BetterMode"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Maximum number of members to migrate",
    )
    parser.add_argument("--circle-token", type=str, help="Circle.so API token")
    parser.add_argument(
        "--circle-community-id", type=str, help="Circle.so community ID"
    )
    parser.add_argument("--bettermode-token", type=str, help="BetterMode API token")
    parser.add_argument(
        "--bettermode-network-id", type=str, help="BetterMode network ID"
    )
    ...
    circle_community_id = args.circle_community_id or os.environ.get(
        "CIRCLE_COMMUNITY_ID"
    )
    bettermode_token = args.bettermode_token or os.environ.get("BETTERMODE_API_TOKEN")
    bettermode_network_id = args.bettermode_network_id or os.environ.get(
        "BETTERMODE_NETWORK_ID"
    )
    ...
    bettermode_config = BetterModeIntegrationConfig(
        api_token=bettermode_token, network_id=bettermode_network_id
    )
