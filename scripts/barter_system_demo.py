...
class BarterSystemDemo:
    ...
    async def create_demo_profiles(self):
        ...
        # Store profiles
        self.entities["wellness_center"] = await self.barter_service.update_profile(wellness_profile)
        self.entities["art_gallery"] = await self.barter_service.update_profile(art_profile)
        self.entities["tech_consultant"] = await self.barter_service.update_profile(practitioner_profile)
    ...
    async def create_demo_listings(self):
        ...
        # Store listings
        self.listings["wellness"] = await self.barter_service.create_listing(wellness_listing)
    ...
    async def create_demo_requests(self):
        ...
        # Store requests
        self.requests["marketing"] = await self.barter_service.create_request(marketing_request)
        self.requests["wellness"] = await self.barter_service.create_request(wellness_request)
        ...
        for request_name, request in self.requests.items():
            ...
            if matches:
                ...
                for i, match in enumerate(matches, 1):
                    ...
                    print(f"     Distance: {match.distance_km:.1f}km" if match.distance_km else "     Virtual available")
                    print(f"     Category match: {'Yes' if match.category_match else 'No'}")
    ...
    async def demonstrate_notifications(self):
        ...
        for entity_id in ["wellness_center_sf", "tech_consultant_la"]:
            notifications = await self.notification_service.get_user_notifications(entity_id, limit=5)
    ...
    async def show_system_statistics(self):
        ...
        print(f"Matches found: {sum(len(matches) for matches in self.matches.values())}")
        ...
        print(f"Cultural adaptation: {'Enabled' if self.config.enable_cultural_adaptation else 'Disabled'}")
    ...
    def print_demo_data(self):
        ...
        demo_data = {
            ...
            "matches": {k: [m.model_dump() for m in matches] for k, matches in self.matches.items()},
...
async def main():
    ...
    try:
        ...
        if input("\nWould you like to see the detailed demo data? (y/n): ").lower() == 'y':
