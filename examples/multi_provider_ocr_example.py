...
from models.tesseract_models import OCRDocumentType, OCRProvider
...
class MultiProviderOCRAgent(BaseAgent, OCRMixin):
    ...
    async def process_important_document(
        ...
        """
        logger.info(
            f"Processing important document of type {document_type}: {image_path}"
        )
...
async def main():
    ...
    if not os.path.exists(image_path):
        ...
        await agent.process_important_document(
            image_path, OCRDocumentType.INVOICE.value
        )
        await agent.process_important_document(
            image_path, OCRDocumentType.PASSPORT.value
        )
