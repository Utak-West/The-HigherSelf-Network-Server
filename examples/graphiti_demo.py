...
async def demo_basic_graphiti_operations():
    ...
    if context:
        logger.info(f"✅ Retrieved context with {len(context.get('search_results', []))} search results")
...
async def demo_agent_workflow_integration():
    ...
    logger.info(f"5. Gallery context retrieved: {len(gallery_context.get('search_results', []))} facts")
...
async def main():
    ...
    logger.info("🚀 Starting Graphiti Integration Demo for The HigherSelf Network Server")
    
    # Check if Graphiti is enabled
    if os.environ.get("GRAPHITI_ENABLED", "true").lower() != "true":
        logger.error("Graphiti is disabled. Please set GRAPHITI_ENABLED=true in your .env file")
        return
