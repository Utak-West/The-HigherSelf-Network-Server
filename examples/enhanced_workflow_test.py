...
def print_transition_result(
    title: str, 
    result: Union[TransitionResult, Tuple[bool, Optional[WorkflowInstance]]]
) -> None:
...
async def setup_enhanced_transitions(workflow: WorkflowStateMachine) -> List[StateTransition]:
    """
...
async def run_enhanced_workflow(workflow: WorkflowStateMachine, enhanced_transitions: List[StateTransition]) -> None:
    ...
    conditional_transition = next(
        (t for t in enhanced_transitions if t.name == "conditional_routing"), 
        None
    )
