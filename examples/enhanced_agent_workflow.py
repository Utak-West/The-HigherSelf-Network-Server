...
async def run_lead_to_booking_workflow():
    ...
    task_data = {
        ...
        "contact_id": lead_result.get("nyra", {}).get(
    ...
    logger.info(
        f"Follow-up task created by Ruvo: {task_result.get('ruvo', {}).get('status')}"
    )
    ...
    logger.info(
        f"Booking processed by Solari: {booking_result.get('solari', {}).get('status')}"
    )
    ...
    workflow_data = {
        ...
        "booking_id": booking_result.get("solari", {}).get(
    ...
    workflow_result = await grace.route_event(
        "workflow_booking_confirmed", workflow_data
    )
...
async def log_message(message: AgentMessage):
    ...
    logger.info(
        f"Message Bus: {message.message_type} from {message.sender} to {message.recipient}"
    )
