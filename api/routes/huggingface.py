...
from integrations.huggingface.models import (HuggingFaceAgent,
                                             HuggingFaceModelReference,
                                             HuggingFaceSpace, ModelType,
                                             SpaceFramework)
...
async def list_models(
    search: Optional[str] = None, task: Optional[str] = None, limit: Optional[int] = 50
):
...
async def sync_model_to_notion(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error syncing model to Notion: {str(e)}"
        )
...
async def sync_space_to_notion(
    request: SpaceRequest, background_tasks: BackgroundTasks
):
    ...
    try:
        ...
        if request.framework:
            try:
                ...
                raise HTTPException(
                    status_code=400, detail=f"Invalid framework: {request.framework}"
                )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error syncing space to Notion: {str(e)}"
        )
