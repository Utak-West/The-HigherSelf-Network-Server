...
async def get_redis_health_async():
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, detail=f"Async Redis health check failed: {str(e)}"
        )
...
async def get_redis_metrics():
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, detail=f"Redis metrics collection failed: {str(e)}"
        )
...
async def get_redis_config():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Redis configuration retrieval failed: {str(e)}"
        )
...
async def test_redis_connection():
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, detail=f"Redis connection test failed: {str(e)}"
        )
...
async def test_redis_async_connection():
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, detail=f"Redis async connection test failed: {str(e)}"
        )
...
async def get_redis_info(
    ...
    try:
        ...
        return {"section": section, "info": info, "timestamp": datetime.now()}

    except Exception as e:
        logger.error(f"Failed to get Redis info for section '{section}': {e}")
        raise HTTPException(
            status_code=503, detail=f"Redis info retrieval failed: {str(e)}"
        )
...
async def get_redis_keys_by_pattern(
    ...
    try:
        ...
        if hasattr(redis_service._sync_client, "scan_iter"):
            keys = list(
                redis_service._sync_client.scan_iter(match=pattern, count=limit)
            )[:limit]
        else:
            raise Exception("Redis client not available")
        ...
        raise HTTPException(
            status_code=503, detail=f"Redis keys retrieval failed: {str(e)}"
        )
