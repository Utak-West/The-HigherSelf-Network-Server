...
from api.webhooks import (WebhookResponse, get_lead_capture_agent,
                          get_marketing_agent, is_test_mode)
...
async def verify_beehiiv_webhook(
    ...
    if is_test_mode():
        logger.info(
            "Running in TEST_MODE: Skipping Beehiiv webhook signature verification"
        )
        return True
    ...
    computed_signature = hmac.new(
        beehiiv_secret.encode("utf-8"), body, hashlib.sha256
    ).hexdigest()
...
async def beehiiv_metrics_webhook(
    ...
    try:
        ...
        if is_test_mode():
            logger.info(
                "Running in TEST_MODE: Simulating Beehiiv metrics webhook processing"
            )
        ...
        return WebhookResponse(
            success=True, message="Successfully processed Beehiiv metrics", data=result
        )
...
async def beehiiv_subscriber_webhook(
    ...
    try:
        ...
        if is_test_mode():
            logger.info(
                "Running in TEST_MODE: Simulating Beehiiv subscriber webhook processing"
            )
            return WebhookResponse(
                success=True,
                message="Beehiiv subscriber webhook simulated successfully (TEST MODE)",
                data={"email": "test@example.com", "action": "subscribed"},
            )
...
async def beehiiv_referral_webhook(
    ...
    try:
        ...
        if is_test_mode():
            logger.info(
                "Running in TEST_MODE: Simulating Beehiiv referral webhook processing"
            )
            return WebhookResponse(
                success=True,
        ...
        logger.info(
            f"Received Beehiiv referral webhook: {referrer_email} referred {referred_email}"
        )
        ...
        referrer_update = await lead_agent.add_referral_to_lead(
