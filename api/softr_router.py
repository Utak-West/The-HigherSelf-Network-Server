...
from models.softr_models import (
    AgentRequest, AgentResponse, AgentInteraction, StaffUser,
    AgentListResponse, AgentInteractionResponse, AgentHistoryResponse,
    SoftrWebhookPayload
)
...
from utils.auth import verify_webhook_signature, get_staff_user
...
async def list_available_agents(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error listing available agents: {str(e)}")
...
async def interact_with_agent(
    ...
    if request.staff_id != staff_user.id:
        raise HTTPException(status_code=403, detail="Staff ID in request does not match authenticated user")
...
async def get_interaction_history(
    ...
    try:
        ...
        interactions = await softr_service.get_staff_agent_history(staff_user.id, agent_id)
        
        return AgentHistoryResponse(interactions=interactions)
        
    except Exception as e:
        logger.error(f"Error retrieving interaction history: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error retrieving interaction history: {str(e)}")
...
async def softr_webhook(
    ...
    try:
        ...
        if event_type == "new_staff_request":
            ...
            if not agent:
                logger.warning(f"Agent {request.agent_id} not found for webhook request")
                return {"status": "error", "message": f"Agent {request.agent_id} not found"}
