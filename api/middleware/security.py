...
class RateLimitConfig(BaseModel):
    ...
    requests_per_minute: int = Field(60, description="Number of requests allowed per minute")
    burst: int = Field(10, description="Burst capacity for request spikes")
    key_prefix: str = Field("rate_limit", description="Redis key prefix for rate limiting")
    
class SecurityConfig(BaseModel):
    """Security configuration for the API gateway."""
    rate_limit: RateLimitConfig = Field(default_factory=RateLimitConfig)
    whitelisted_ips: List[str] = Field(default_factory=list, description="IP addresses exempt from rate limiting")
    blacklisted_ips: List[str] = Field(default_factory=list, description="Blocked IP addresses")
    sensitive_endpoints: List[str] = Field(default_factory=list, description="Endpoints requiring additional protection")
    api_key_header_name: str = Field("X-API-Key", description="Header name for API key authentication")
...
class RequestValidationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if request.headers.get("content-length") and int(request.headers["content-length"]) > 10_485_760:
            REQUEST_REJECTED.labels(
                method=request.method,
                endpoint=request.url.path,
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        await redis_service.async_set(rate_key, recent_requests, ex=120)  # 2 minute expiry
