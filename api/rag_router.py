...
from knowledge.rag_pipeline import get_rag_pipeline, RAGRequest, RAGResponse, RAGPipeline, SourceReference
...
class RAGCompletionRequest(BaseModel):
    ...
    content_types: Optional[List[str]] = Field(None, description="Optional filter for content types")
    notion_database_ids: Optional[List[str]] = Field(None, description="Optional filter for Notion database IDs")
    search_limit: int = Field(5, description="Maximum number of search results to use")
    similarity_threshold: float = Field(0.7, description="Similarity threshold for search results")
    system_message: Optional[str] = Field(None, description="Optional system message for the AI")
...
class RAGCompletionResponse(BaseModel):
    ...
    sources: List[SourceReference] = Field([], description="Sources used for generation")
