...
async def circleso_new_member_webhook(
    ...
    try:
        ...
        if is_test_mode():
            logger.info("Running in TEST_MODE: Simulating Circle.so new member webhook processing")
        ...
        logger.info(f"Received Circle.so new member webhook for: {member_data['email']}")
...
async def circleso_activity_webhook(
    ...
    try:
        # Check if in test mode
        if is_test_mode():
            logger.info("Running in TEST_MODE: Simulating Circle.so activity webhook processing")
            return WebhookResponse(
                success=True,
        ...
        logger.info(f"Received Circle.so activity webhook for: {activity_data['member_email']}")
...
async def circleso_event_webhook(
    ...
    try:
        # Check if in test mode
        if is_test_mode():
            logger.info("Running in TEST_MODE: Simulating Circle.so event webhook processing")
            return WebhookResponse(
                success=True,
