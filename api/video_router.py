...
from models.video_models import VideoGenerationConfig, VideoContent, VideoStatus
...
class VideoGenerationRequest(BaseModel):
    ...
    resolution: str = Field("1080x1920", description="Video resolution (1080x1920 for portrait, 1920x1080 for landscape)")
    ...
    business_entity_id: str = Field("the_connection_practice", description="Business entity ID for tracking")
...
@router.post("/generate", response_model=VideoGenerationResponse, dependencies=[Depends(verify_api_key)])
...
@router.get("/{content_id}/status", response_model=VideoStatusResponse, dependencies=[Depends(verify_api_key)])
async def get_video_status(
    ...
    try:
        ...
        if result.get("status") == "error":
            raise HTTPException(status_code=404, detail=result.get("message", "Video content not found"))
