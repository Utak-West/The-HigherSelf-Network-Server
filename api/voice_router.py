...
from services.aqua_voice_service import get_aqua_voice_service, VoiceTranscriptionRequest, VoiceCommandRequest, AquaVoiceService
...
class TranscriptionRequest(BaseModel):
    ...
    context: Optional[str] = Field(None, description="Optional context for better transcription")
    format_type: Optional[str] = Field(None, description="Optional formatting type (email, code, etc.)")
...
class CommandRequest(BaseModel):
    ...
    text: str = Field(..., description="Transcribed text")
    context: Optional[str] = Field(None, description="Optional context for command processing")


class CommandResponse(BaseModel):
    """Response for command requests."""
    success: bool = Field(..., description="Whether the command processing was successful")
    is_command: bool = Field(..., description="Whether the text was identified as a command")
    text: str = Field(..., description="Original text")
    response: Optional[str] = Field(None, description="Response to the command")
    action: Optional[Dict[str, Any]] = Field(None, description="Structured action details")
    error: Optional[str] = Field(None, description="Error message if command processing failed")
...
async def transcribe_and_process_command(
    ...
    try:
        ...
        transcription_result = await voice_service.transcribe(voice_transcription_request)
