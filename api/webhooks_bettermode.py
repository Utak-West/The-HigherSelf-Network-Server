...
from services.bettermode_service import get_bettermode_service, BetterModeService
...
async def verify_bettermode_webhook(
    request: Request,
    x_bettermode_signature: Optional[str] = Header(None)
) -> None:
    """
...
@router.post("/member", response_model=WebhookResponse, dependencies=[Depends(verify_bettermode_webhook)])
async def bettermode_member_webhook(
    ...
    try:
        ...
        if is_test_mode():
            logger.info("Running in TEST_MODE: Simulating BetterMode member webhook processing")
        ...
        if webhook_payload.event_type == BetterModeWebhookType.MEMBER_CREATED:
            ...
            logger.info(f"Received BetterMode new member webhook for: {member_data['email']}")
            ...
            logger.info(f"Received BetterMode member update webhook for: {webhook_payload.data.get('email')}")
            ...
            logger.warning(f"Unhandled BetterMode member event type: {webhook_payload.event_type}")
            return WebhookResponse(
                success=False,
...
@router.post("/activity", response_model=WebhookResponse, dependencies=[Depends(verify_bettermode_webhook)])
async def bettermode_activity_webhook(
    ...
    try:
        # Check if in test mode
        if is_test_mode():
            logger.info("Running in TEST_MODE: Simulating BetterMode activity webhook processing")
            return WebhookResponse(
                success=True,
        ...
        if webhook_payload.event_type in [
            ...
            member_id = webhook_payload.data.get("authorId") or webhook_payload.data.get("memberId")
            if not member_id:
                logger.warning(f"No member ID found in BetterMode activity webhook: {webhook_payload.event_type}")
                return WebhookResponse(
                    success=False,
            ...
            if not member:
                logger.warning(f"Member not found for ID {member_id} in BetterMode activity webhook")
                return WebhookResponse(
                    success=False,
            ...
            logger.info(f"Received BetterMode activity webhook for: {activity_data['member_email']}")
            ...
            logger.warning(f"Unhandled BetterMode activity event type: {webhook_payload.event_type}")
            return WebhookResponse(
                success=False,
...
@router.post("/event", response_model=WebhookResponse, dependencies=[Depends(verify_bettermode_webhook)])
async def bettermode_event_webhook(
    ...
    try:
        # Check if in test mode
        if is_test_mode():
            logger.info("Running in TEST_MODE: Simulating BetterMode event webhook processing")
            return WebhookResponse(
                success=True,
