                ...
                if client_id in connections:
                    ...
                    await mongo_service.async_insert_one(
                        "agent_websocket_disconnections",
                        {
                            "agent_id": agent_id,
                            "client_id": client_id,
                            "connected_at": self.connection_info.get(client_id, {}).get("connected_at"),
                            "disconnected_at": datetime.utcnow().isoformat(),
                            "duration_seconds": (
                                datetime.utcnow() - 
                                datetime.fromisoformat(self.connection_info.get(client_id, {}).get("connected_at", datetime.utcnow().isoformat()))
                            ).total_seconds() if self.connection_info.get(client_id, {}).get("connected_at") else 0
                        }
                    )
...
async def get_agent(agent_id: str = Query(...)) -> Optional[Agent]:
    ...
    try:
        agent_data = await mongo_service.async_find_one("agents", {"agent_id": agent_id})
        if agent_data:
...
async def websocket_agent_endpoint(
    ...
    if not client_id:
        client_id = f"{agent_id}-{websocket.client.host}-{datetime.utcnow().timestamp()}"
...
async def websocket_workflow_endpoint(
    ...
    workflow = await mongo_service.async_find_one("workflows", {"workflow_id": workflow_id})
    if not workflow:
