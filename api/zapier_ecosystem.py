...
from services.zapier_ecosystem_service import (ZapierComponentType,
...
async def setup_complete_ecosystem(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to setup Zapier ecosystem: {str(e)}"
        )
...
async def sync_table_with_notion(
    ...
    try:
        if is_api_disabled():
            logger.info(
                f"API disabled: Simulating table sync for {sync_request.table_id}"
            )
            return ZapierEcosystemResponse(
                success=True,
        ...
        raise HTTPException(status_code=500, detail=f"Failed to sync table: {str(e)}")


@router.get("/status", response_model=ZapierEcosystemResponse)
async def get_ecosystem_status(authenticated: bool = Depends(verify_staff_api_key)):
    """
    ...
    try:
        ...
        status_data = {
            "connection_practice": {
                "tables": len(
                    [
                        t
                        for t in zapier_service.ecosystem_config.tables
                ...
                "interfaces": len(
                    [
                        i
                        for i in zapier_service.ecosystem_config.interfaces
                ...
                "chatbots": len(
                    [
                        c
                        for c in zapier_service.ecosystem_config.chatbots
                ...
                "canvases": len(
                    [
                        c
                        for c in zapier_service.ecosystem_config.canvases
                        if c.entity_area == ZapierEntityArea.CONNECTION_PRACTICE
                    ]
                ),
                "agents": len(
                    [
                        a
                        for a in zapier_service.ecosystem_config.agents
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get ecosystem status: {str(e)}"
        )


@router.post("/webhook", response_model=ZapierEcosystemResponse)
async def handle_zapier_webhook(request: Request, payload: ZapierWebhookPayload):
    """
    ...
    try:
        if is_api_disabled():
            logger.info(
                f"API disabled: Simulating webhook processing for {payload.component_type}"
            )
            return ZapierEcosystemResponse(
                success=True,
...
async def process_zapier_webhook(payload: ZapierWebhookPayload) -> Dict[str, Any]:
    ...
    """
    logger.info(
        f"Processing Zapier webhook: {payload.component_type} - {payload.action}"
    )
