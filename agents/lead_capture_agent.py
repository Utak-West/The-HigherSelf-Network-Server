...
class LeadCaptureAgent(BaseAgent):
    ...
    def __init__(
        ...
        apis_utilized = [ApiPlatform.NOTION, ApiPlatform.TYPEFORM, ApiPlatform.HUBSPOT]
    ...
    async def _typeform_to_standardized_lead(
        self, submission: TypeformSubmission
    ) -> StandardizedLead:
        """
        ...
        if not email:
            raise ValueError(
                "Email is required but was not found in the Typeform submission"
            )
    ...
    async def _website_form_to_standardized_lead(
        self, submission: WebsiteFormSubmission
    ) -> StandardizedLead:
        """
        ...
        interests = []
        if "interests" in submission.fields and isinstance(
            submission.fields["interests"], list
        ):
        ...
        additional_data = {
            k: v
            for k, v in submission.fields.items()
            if k not in ["email", "first_name", "last_name", "phone", "interests"]
        }

        if not email:
            raise ValueError(
                "Email is required but was not found in the website form submission"
            )
    ...
    async def _generic_event_to_standardized_lead(
        ...
        if not email:
            raise ValueError(
                "Email is required but was not found in the generic event data"
            )
        ...
        source_id = event_data.get(
            "event_id"
        )  # This would be the Zapier-provided ID or event-specific ID
        if not source_id:
            ...
            logger.warning(
                f"Generated fallback source_id for generic event: {source_id} for email: {email}"
            )
        ...
        if event_data.get(
            "raw_payload"
        ):  # Storing the raw payload is good for debugging
        ...
        if event_data.get("integration_source"):  # As set in api/server.py
            additional_data_payload["integration_source"] = event_data.get(
                "integration_source"
            )
        ...
        phone = additional_data_payload.pop(
            "phone", event_data.get("phone")
        )  # Check top-level too
        interests_data = additional_data_payload.pop(
        ...
        if isinstance(interests_data, list):
            ...
            interests = [
                s.strip() for s in interests_data.split(",")
    ...
    async def _check_for_duplicates(
        ...
        filter_conditions = {
            ...
            "rich_text": {"equals": lead.email.lower()},
        }
        ...
        for instance in existing_instances:
            ...
            if instance.start_date:  # Check if start_date is not None
                try:
                    hours_diff = (
                        datetime.now() - instance.start_date
                    ).total_seconds() / 3600
                    if hours_diff <= self.lead_deduplication_window_hours:
                        ...
                        if (
                            ...
                            and lead.source
                            and (
                                instance.source_system.lower() == lead.source.lower()
                                or lead.source.lower() in instance.source_system.lower()
                                or instance.source_system.lower() in lead.source.lower()
                            )
                        ):
                            self.logger.info(
                                f"Found duplicate lead: {lead.email} from {lead.source} (matches existing {instance.source_system})"
                            )
                            return instance
                except TypeError as e:
                    # This can happen if one datetime is naive and the other is aware
                    self.logger.error(
                        f"TypeError during date comparison for duplicate check (lead: {lead.email}): {e}. Instance start_date: {instance.start_date}, now: {datetime.now()}"
                    )
    ...
    async def _create_workflow_instance(
        self, lead: StandardizedLead, business_entity_id: str, workflow_id: str
    ) -> WorkflowInstance:
        """
        ...
        instance = WorkflowInstance(
            ...
            or lead.email,  # Fallback to email if name is empty
        ...
        self.logger.info(
            f"Created workflow instance (ID: {instance.page_id}) for lead: {lead.email} in workflow: {workflow_id}"
        )
    ...
    async def process_typeform_webhook(
        ...
        """
        self.logger.info(
            f"Processing Typeform webhook for form: {payload.get('form_id')}"
        )
        ...
        try:
            submission = TypeformSubmission(
                ...
                submitted_at=datetime.fromisoformat(
                    payload.get("submitted_at").replace("Z", "+00:00")
            ...
            self.logger.error(
                f"Error parsing Typeform submission: {e} - Payload: {payload}"
            )
            return {
                "status": "error",
                "message": f"Error parsing Typeform data: {str(e)}",
            }
        ...
        instance = await self._create_workflow_instance(
            lead, business_entity_id, workflow_id
        )
    ...
    async def process_generic_event_registration(
        ...
        event_data: Dict[
            str, Any
        ],  # This is the event_data_for_agent from api/server.py
    ) -> Dict[str, Any]:
        """
        Process a generic event registration (e.g., from Zapier).
        """
        self.logger.info(
            f"Processing generic event registration from: {event_data.get('source_platform', 'Unknown Source')} for: {event_data.get('email')}"
        )
    ...
    async def process_website_form(
        ...
        try:
            ...
            self.logger.error(
                f"Error parsing website form submission: {e} - Data: {form_data}"
            )
            return {
                "status": "error",
                "message": f"Error parsing website form data: {str(e)}",
            }
    ...
    async def process_event(
        ...
        """
        self.logger.info(
            f"LeadCaptureAgent received event: {event_type} with data: {event_data}"
        )
        ...
        try:
            if event_type == "typeform_webhook":
                ...
                return await self.process_generic_event_registration(
                    event_data=event_data
                )
            else:
                self.logger.warning(
                    f"Unhandled event type in LeadCaptureAgent: {event_type}"
                )
                return {
                    "status": "error",
            ...
            self.logger.exception(
                f"Error processing event in LeadCaptureAgent ({event_type}): {e}"
            )
            return {
                "status": "error",
    ...
    async def check_health(self) -> Dict[str, Any]:
        ...
        if overall_status == "healthy" and not (
            health_checks["typeform_api"] and health_checks["hubspot_api"]
        ):
