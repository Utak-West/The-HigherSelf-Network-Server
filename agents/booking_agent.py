...
class BookingAgent(BaseAgent):
    ...
    async def _check_for_existing_workflow(self, booking_id: str) -> Optional[WorkflowInstance]:
        """
    ...
    async def process_booking(
        ...
        if existing_instance:
            self.logger.info(f"Booking {booking.booking_id} already has a workflow instance")
        ...
        key_data = {
            ...
            "payment_status": booking.payment_info.get("status", "unknown") if booking.payment_info else "unknown",
    ...
    async def update_booking_status(
        ...
        if not existing_instance:
            self.logger.error(f"No workflow instance found for booking ID: {booking_id}")
            return {
                "status": "error",
        ...
        if not new_workflow_state:
            self.logger.warning(f"Unknown booking status: {new_status}, staying in current state")
