...
from models.notion_db_models import WorkflowInstance, Task, BusinessEntity
from models.task_models import (
    TaskPriority, TaskStatus, TaskSource, 
    TaskCategory, TaskTemplate, TaskReminder
)
...
class TaskManagementAgent(BaseAgent):
    ...
    Uses Notion as the central hub for all task data.
    """
    ...
    def __init__(
        ...
        capabilities = [
            ...
            AgentCapability.CALENDAR_INTEGRATION,
            AgentCapability.WORKFLOW_MANAGEMENT
        ]
        
        apis_utilized = [
            ApiPlatform.NOTION,
            ApiPlatform.GOOGLE_CALENDAR
        ]
    ...
    async def create_task_from_template(
        ...
        Creates the task in Notion as the central hub.
        """
        ...
        self.logger.info(f"Created task {task_id} in Notion from template {template_id}")
    ...
    async def create_task(
        ...
        if reminder_days_before and due_date:
            remind_at = datetime.combine(due_date, datetime.min.time()) - timedelta(days=reminder_days_before)
        ...
        self.logger.info(f"Created task {task_id} in Notion")
    ...
    async def update_task_status(
        ...
        Updates the task in Notion as the central hub.
        """
        ...
        if status == TaskStatus.COMPLETED and task.workflow_instance_id:
            workflow_instance = await notion_svc.get_workflow_instance(task.workflow_instance_id)
            if workflow_instance:
                await self.log_action(
                    workflow_instance=workflow_instance,
        ...
        self.logger.info(f"Updated task {task_id} status from {previous_status} to {status.value}")
    ...
    async def process_workflow_event(
        ...
        Creates tasks in Notion as the central hub.
        """
        self.logger.info(f"Processing workflow event {event_type} for instance {workflow_instance_id}")
        
        notion_svc = await self.notion_service
        workflow_instance = await notion_svc.get_workflow_instance(workflow_instance_id)
        
        if not workflow_instance:
            return {"status": "error", "message": f"Workflow instance {workflow_instance_id} not found"}
        ...
        if not templates:
            self.logger.info(f"No task templates found for workflow {workflow_instance.workflow_id} and event {event_type}")
            return {
                "status": "success",
        ...
        self.logger.info(f"Created {len(created_tasks)} tasks for workflow instance {workflow_instance_id}")
    ...
    def _calculate_offset(self, template: TaskTemplate, event_data: Dict[str, Any]) -> Optional[int]:
    ...
    async def check_health(self) -> Dict[str, Any]:
        ...
        health_checks["google_calendar_api"] = "GOOGLE_CALENDAR_API_KEY" in self.api_keys
