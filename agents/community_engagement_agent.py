...
from models.base import ApiPlatform, AgentCapability
...
class CommunityEngagementAgent(BaseAgent):
    ...
    def __init__(self, notion_service: NotionService):
        ...
        super().__init__(
            ...
            apis_utilized=[
                ApiPlatform.NOTION  # Primary data store
            ]
        )
    ...
    async def track_member_activity(self, activity_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(f"Tracking community activity for: {activity_data.get('member_email')}")
        
        # Step 1: Find the member in the Community Hub DB
        member = await self._find_member_by_email(activity_data.get('member_email'))
        if not member:
            logger.warning(f"Community member not found: {activity_data.get('member_email')}")
            return {
                "success": False,
        ...
        if member.engagement_score is None:
            ...
            points = {
                ...
                'reaction': 2,
                'login': 1
            }.get(activity_data.get('activity_type').lower(), 1)
    ...
    async def schedule_community_event(self, event_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        target_members = await self._find_members_for_event(event_data.get('target_criteria', {}))
        
        # Step 3: Send event notifications
        notification_count = 0
        for member in target_members:
            notification_sent = await self._notify_member_about_event(member, event_data)
    ...
    async def _find_or_create_contact(self, member_data: Dict[str, Any]) -> ContactProfile:
        """
    ...
    async def _send_welcome_notification(self, member: CommunityMember) -> Dict[str, Any]:
        """
        ...
        if is_api_disabled("circle"):
            ...
            logger.info(f"[TESTING MODE] Simulated sending welcome message to {member.member_email}")
            return {
                "success": True,
    ...
    async def _update_contact_engagement(self, contact_id: str, activity_data: Dict[str, Any]):
        """
    ...
    async def _find_members_for_event(self, criteria: Dict[str, Any]) -> List[CommunityMember]:
        """
        ...
        if "membership_level" in criteria:
            ...
            level_filter = {
                ...
                "select": {
                    "equals": criteria["membership_level"]
                }
            }
    ...
    async def _notify_member_about_event(
        ...
        if is_api_disabled("circle"):
            ...
            logger.info(f"[TESTING MODE] Simulated sending event notification to {member.member_email}")
            return True
            
        # In a real implementation, this would use a NotificationService
        # and potentially the NotificationTemplate model from Notion
        logger.info(f"Notifying {member.member_email} about event: {event_data.get('title')}")
