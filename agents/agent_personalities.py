...
class GraceOrchestrator:
    ...
    def __init__(self, agents: List[BaseAgent], message_bus: Optional[MessageBus] = None):
        """
        ...
        self.workflow_patterns = {
            ...
            "lead_to_booking": {
                ...
                "steps": [
                    {"agent": "Nyra", "event": "lead_capture", "next_on_success": "lead_enrichment"},
                    {"agent": "Nyra", "event": "lead_enrichment", "next_on_success": "create_task"},
                    {"agent": "Ruvo", "event": "create_task", "next_on_success": "campaign_trigger"},
                    {"agent": "Liora", "event": "campaign_trigger", "next_on_success": None}
                ]
            },
            ...
            "content_lifecycle": {
                ...
                "steps": [
                    {"agent": "Elan", "event": "content_idea", "next_on_success": "content_draft"},
                    {"agent": "Elan", "event": "content_draft", "next_on_success": "content_review"},
                    {"agent": "Elan", "event": "content_review", "next_on_success": "content_publish"},
                    {"agent": "Elan", "event": "content_publish", "next_on_success": "campaign_trigger"},
                    {"agent": "Liora", "event": "campaign_trigger", "next_on_success": "audience_analysis"},
                    {"agent": "Zevi", "event": "audience_analysis", "next_on_success": None}
                ]
            },
            ...
            "retreat_booking": {
                ...
                "steps": [
                    {"agent": "Solari", "event": "retreat_registration", "next_on_success": "payment_received"},
                    {"agent": "Solari", "event": "payment_received", "next_on_success": "create_task"},
                    {"agent": "Ruvo", "event": "create_task", "next_on_success": "appointment_reminder"},
                    {"agent": "Solari", "event": "appointment_reminder", "next_on_success": "community_event"},
                    {"agent": "Sage", "event": "community_event", "next_on_success": None}
                ]
            }
        }
        ...
        if self.message_bus:
            self.message_bus.subscribe("GraceOrchestrator", self.process_message)

        self.logger.info("✨ Grace Fields Orchestrator initialized with enhanced capabilities")
        self.logger.info(f"Coordinating {len(self.agents)} agents: {', '.join(self.agents.keys())}")
        if self.agent_capabilities:
            self.logger.info(f"Registered {len(self.agent_capabilities)} agent capabilities for dynamic routing")
    ...
    async def route_event(self, event_type: str, event_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.logger.info(f"Grace Fields processing event: {event_type} [tracking: {tracking_id}]")
        ...
        if "workflow_id" in event_data and event_data.get("workflow_id") in self.active_workflows:
            return await self.continue_workflow(event_data["workflow_id"], event_type, event_data)
        ...
        if business_entity:
            ...
            for name, agent in self.agents.items():
                if hasattr(agent, "business_entities") and business_entity in agent.business_entities:
                    if await self._can_agent_handle_event(agent, event_type):
                        self.logger.info(f"Business entity routing: {event_type} for {business_entity} to {name}")
        ...
        if "_" in event_type:
            ...
            for pattern, agent in [
                ...
                if domain.startswith(pattern) and agent in self.agents:
                    self.logger.info(f"Pattern-based routing: {event_type} to {agent} based on '{pattern}' prefix")
            ...
            if hasattr(agent, "process_event"):
                try:
                    ...
                    if agent_result.get("status") != "error":
                        ...
                        self.logger.info(f"Dynamic routing: {event_type} to {agent_name}")
                        ...
                        if self.message_bus:
                            await self.publish_event_result(event_type, event_data, agent_name, agent_result)
        ...
        if hasattr(self, "ai_router") and self.ai_router:
            try:
                ai_routing_result = await self.ai_router.route_event(event_type, event_data)
    ...
    async def _route_to_agent(self, agent_name: str, event_type: str, event_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            agent_result = await self.agents[agent_name].process_event(event_type, event_data)
            results[agent_name.lower()] = agent_result
            ...
            await self._check_for_workflow_triggers(event_type, event_data, agent_name, agent_result)
            ...
            fallback_result = await self._try_fallback_routing(event_type, event_data, agent_name)
            if fallback_result:
    ...
    async def _try_capability_routing(self, event_type: str, event_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if required_capability and required_capability in self.agent_capabilities:
            ...
            if capable_agents:
                ...
                self.logger.info(f"Capability-based routing: {event_type} to {agent_name} with {required_capability}")
                return await self._route_to_agent(agent_name, event_type, event_data)

        return None

    async def _try_fallback_routing(self, event_type: str, event_data: Dict[str, Any], failed_agent: str) -> Optional[Dict[str, Any]]:
        ...
        if failed_agent in fallbacks:
            for fallback_agent in fallbacks[failed_agent]:
                if fallback_agent in self.agents:
                    try:
                        self.logger.info(f"Trying fallback from {failed_agent} to {fallback_agent} for {event_type}")
                        result = await self.agents[fallback_agent].process_event(event_type, event_data)
    ...
    async def _check_for_workflow_triggers(self, event_type: str, event_data: Dict[str, Any],
                                          agent_name: str, agent_result: Dict[str, Any]) -> None:
        ...
        if event_type in workflow_triggers and agent_result.get("status") != "error":
            ...
            if workflow_name in self.workflow_patterns:
                workflow_data = {**event_data, "triggered_by": event_type, "source_agent": agent_name}
                await self.start_workflow_pattern(workflow_name, workflow_data)

    async def start_workflow_pattern(self, pattern_name: str, event_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        self.logger.info(f"Grace Fields starting workflow pattern: {pattern_name} [ID: {workflow_id}]")
        ...
        try:
            if agent_name in self.agents:
                step_result = await self.agents[agent_name].process_event(event_type, step_data)
                ...
                if step_result.get("status") != "error" and first_step["next_on_success"]:
                    # Queue the next step for async execution
                    asyncio.create_task(self._continue_workflow_async(workflow_id, first_step["next_on_success"]))
    ...
    async def _continue_workflow_async(self, workflow_id: str, next_event: str) -> None:
        ...
        if workflow_id not in self.active_workflows:
            self.logger.error(f"Cannot continue workflow {workflow_id}: not found in active workflows")
            return
        ...
        try:
            if agent_name in self.agents:
                self.logger.info(f"Continuing workflow {workflow_id} with step {next_step}: {agent_name}.{event_type}")
                step_result = await self.agents[agent_name].process_event(event_type, step_data)

                # Update workflow with results
                workflow["results"][f"step_{next_step}_{agent_name}_{event_type}"] = step_result
                workflow["steps_completed"].append(next_step)

                # Check if we should continue to the next step
                if step_result.get("status") != "error" and step["next_on_success"]:
                    # Queue the next step for async execution
                    asyncio.create_task(self._continue_workflow_async(workflow_id, step["next_on_success"]))
                elif step_result.get("status") != "error" and next_step == len(pattern["steps"]) - 1:
                ...
                self.logger.error(f"Agent {agent_name} not found for workflow step {next_step}")
    ...
    async def continue_workflow(self, workflow_id: str, event_type: str, event_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if workflow["status"] != "active":
            return {"error": f"Workflow {workflow_id} is not active (status: {workflow['status']})", "status": "error"}
        ...
        if current_step >= len(pattern["steps"]):
            return {"error": f"Workflow {workflow_id} has no more steps", "status": "error"}
        ...
        try:
            if agent_name in self.agents:
                self.logger.info(f"Continuing workflow {workflow_id} with event {event_type} at step {current_step}")
                step_result = await self.agents[agent_name].process_event(event_type, step_data)

                # Update workflow with results
                workflow["results"][f"step_{current_step}_{agent_name}_{event_type}"] = step_result
                ...
                error_msg = f"Agent {agent_name} not found for workflow step {current_step}"
                self.logger.error(error_msg)
                return {"error": error_msg, "status": "error"}
        except Exception as e:
    ...
    async def handle_workflow_event(self, event_type: str, event_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        self.logger.info(f"Grace Fields processing workflow event {event_type} [ID: {workflow_id}]")
        ...
        if event_type == "workflow_lead_capture_complete":
            ...
            results["nyra"] = await self.agents["Nyra"].process_event("lead_capture_complete", event_data)

            # Only continue if lead capture was successful
            if results["nyra"].get("status") != "error":
                results["solari"] = await self.agents["Solari"].process_event("new_lead_notification", event_data)
                ...
                results["ruvo"] = await self.agents["Ruvo"].process_event("create_task", task_data)
            ...
            results["solari"] = await self.agents["Solari"].process_event("booking_confirmed", event_data)
            ...
            if results["solari"].get("status") != "error":
                ...
                results["liora"] = await self.agents["Liora"].process_event("campaign_trigger", campaign_data)
                ...
                results["zevi"] = await self.agents["Zevi"].process_event("segment_update", segment_data)

        elif event_type == "workflow_content_published":
            # When content is published, distribute and create marketing campaign
            results["elan"] = await self.agents["Elan"].process_event("content_publish", event_data)

            # Only continue if content publishing was successful
            if results["elan"].get("status") != "error":
                # Distribute to social media
                results["elan_distribute"] = await self.agents["Elan"].process_event("distribute_content", event_data)
                ...
                results["zevi"] = await self.agents["Zevi"].process_event("audience_analysis", {
                    **event_data,
                    "content_id": event_data.get("content_id"),
            ...
            for agent_name, agent in self.agents.items():
                if await self._can_agent_handle_event(agent, workflow_type):
                    results[agent_name.lower()] = await agent.process_event(workflow_type, event_data)
                    self.logger.info(f"Workflow {workflow_type} handled by {agent_name}")
                    break
    ...
    async def publish_event_result(self, event_type: str, event_data: Dict[str, Any],
                                  agent_name: str, result: Dict[str, Any]) -> None:
        """
    ...
    async def process_message(self, message: AgentMessage) -> None:
        ...
        if message.message_type == "agent_status_update":
            self.logger.info(f"Agent status update from {message.sender}: {message.payload.get('status')}")
            ...
            if workflow_id and transition:
                self.logger.info(f"Workflow transition request: {workflow_id} -> {transition}")
            ...
            for agent in self.agents.values():
                if hasattr(agent, "capabilities") and requested_capability in agent.capabilities:
    ...
    async def check_health(self) -> Dict[str, Any]:
        ...
        agent_results = await asyncio.gather(*health_check_tasks, return_exceptions=True)
...
def create_grace_orchestrator(notion_client, message_bus=None) -> GraceOrchestrator:
    """
    ...
    return GraceOrchestrator(agents=agent_instances, message_bus=message_bus)
