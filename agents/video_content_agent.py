...
from models.content_models import ContentType, ContentStage, ContentPlatform
from models.video_models import VideoContent, VideoStatus, VideoResolution, VideoGenerationConfig
from services.video_generation_service import VideoGenerationService, VideoGenerationRequest
...
class VideoContentAgent:
    ...
    async def generate_video(self, config: VideoGenerationConfig, business_entity_id: str) -> Dict[str, Any]:
        """
        ...
        asyncio.create_task(self._monitor_video_generation(content_id, response.task_id))
    ...
    async def _monitor_video_generation(self, content_id: str, task_id: str) -> None:
        ...
        while not completed:
            try:
                ...
                if completed:
                    self.logger.info(f"Video generation completed for content ID: {content_id}")
                    break
    ...
    async def get_video_status(self, content_id: str) -> Dict[str, Any]:
        ...
        if video_content.task_id:
            try:
                task_status = await self.video_service.get_task_status(video_content.task_id)
