...
from models.tesseract_models import (OCRImageSource, OCRLanguage,
                                     OCROutputFormat, OCRRequest, OCRResponse)
...
class TesseractMixin:
    ...
    def __init__(self):
        ...
        logger.info(
            f"TesseractMixin initialized for {getattr(self, 'agent_id', 'unknown agent')}"
        )
    ...
    async def setup_tesseract(self, tesseract_service=None):
        ...
        logger.info(
            f"Tesseract service set up for {getattr(self, 'agent_id', 'unknown agent')}"
        )
    ...
    async def extract_text_from_image(
        ...
        try:
            ...
            return {"success": False, "error": f"Invalid language code: {str(e)}"}
    ...
    async def extract_text_from_url(
        self, image_url: str, languages: List[str] = ["eng"], preprocessing: bool = True
    ) -> Dict[str, Any]:
        """
