...
from models.notion_db_models_extended import MarketingCampaign, ContactProfile
...
class MarketingCampaignAgent(BaseAgent):
    ...
    async def execute_email_campaign(self, campaign_id: str) -> Dict[str, Any]:
        ...
        if not target_contacts:
            logger.warning(f"No contacts match the targeting criteria for campaign {campaign_id}")
            return {
                "success": False,
        ...
        if campaign.channels and "Email - Beehiiv" in campaign.channels:
            result = await self._send_beehiiv_campaign(campaign, target_contacts, template)
        else:
    ...
    async def track_campaign_metrics(self, campaign_id: str) -> Dict[str, Any]:
        ...
        if campaign.channels and "Email - Beehiiv" in campaign.channels:
            ...
            raise ValueError(f"Unsupported campaign channel for metrics: {campaign.channels}")
    ...
    async def _get_targeted_contacts(self, targeting_criteria: Dict[str, Any]) -> List[ContactProfile]:
        """
    ...
    async def _send_beehiiv_campaign(
        ...
        if is_api_disabled("beehiiv"):
            ...
            logger.info(f"[TESTING MODE] Simulated sending Beehiiv campaign to {len(contacts)} recipients")
            return {
                "provider": "beehiiv",
        ...
        recipients = [
            ...
            for contact in contacts if contact.email
        ]
    ...
    async def _get_beehiiv_metrics(self, campaign: MarketingCampaign) -> Dict[str, Any]:
        ...
        if is_api_disabled("beehiiv"):
            ...
            logger.info(f"[TESTING MODE] Simulated retrieving Beehiiv metrics for campaign {campaign.campaign_id}")
