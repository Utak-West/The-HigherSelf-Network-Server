...
from monitoring.langchain_callbacks import MonitoringCallback, PerformanceCallback
from security.input_validator import SecureInput, OutputFilter
...
class LangChainAgent(BaseAgent, ABC):
    ...
    def _create_agent_executor(self):
        ...
        try:
            ...
            self.logger.info(f"Agent executor created for {self.name} with {len(self.tools)} tools")
        ...
        try:
            ...
            if langchain_config.enable_output_filtering:
                if isinstance(result.get("output"), str):
                    result["output"] = OutputFilter.filter_sensitive_data(result["output"])
        ...
        for tool in self.tools:
            try:
                ...
                if not hasattr(tool, 'name') or not hasattr(tool, 'description'):
                    self.logger.error(f"Invalid tool configuration for {self.name}: {tool}")
                    return False
            except Exception as e:
