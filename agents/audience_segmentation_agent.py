...
from models.notion_db_models import WorkflowInstance, BusinessEntity
from models.audience_models import (
    AudienceSegment, CustomerProfile, SegmentCriteria, 
    InteractionType, SegmentSyncResult
)
...
class AudienceSegmentationAgent(BaseAgent):
    ...
    def __init__(
        ...
        api_key_names = ["WOOCOMMERCE_API_KEY", "AMELIA_API_KEY", "TUTORM_API_KEY", 
                         "BEEHIIV_API_KEY", "SIMPLE_TEXTING_API_KEY"]
    ...
    async def analyze_customer_data(
        ...
        if business_entity:
            ...
            self.logger.info(f"Updated business entity {business_entity_id} with customer data analysis")
    ...
    async def refresh_segment(
        ...
        self.logger.info(f"Refreshed segment {segment_id} in Notion, member count: {new_count}")
    ...
    async def sync_segment_to_platform(
        ...
        if platform_key not in self.api_keys:
            return {"status": "error", "message": f"API key for {platform} not configured"}
        
        # This would sync segment to the external platform in a full implementation
        # For now, simulate sync
        external_id = segment.platform_ids.get(platform, f"{platform}_segment_{segment.name.lower().replace(' ', '_')}")
        ...
        self.logger.info(f"Synced segment {segment_id} to {platform} with external ID {external_id}")
    ...
    async def get_customer_segments(
        ...
        if not customer:
            return {"status": "error", "message": f"Customer with email {email} not found"}
    ...
    async def check_health(self) -> Dict[str, Any]:
        ...
        health_checks["distribution_apis"] = (
            "BEEHIIV_API_KEY" in self.api_keys or 
            "SIMPLE_TEXTING_API_KEY" in self.api_keys
        )
