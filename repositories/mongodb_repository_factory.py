...
class MongoRepositoryFactory:
    ...
    def get_repository_for_model(self, model_class: Type[T]) -> Optional[MongoRepository[T]]:
        """
        ...
        if model_class not in self._model_to_repo:
            logger.warning(f"No repository found for model class '{model_class.__name__}'")
            return None
