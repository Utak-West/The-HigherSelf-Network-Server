...
from models.dataset_models import (DatasetMetadata, DatasetTrainingConfig,
                                   DatasetTrainingResult, DatasetVersion,
                                   ProcessedDataset)
...
class DatasetMetadataRepository(MongoRepository[DatasetMetadata]):
    ...
    def find_by_openml_id(self, openml_id: str) -> Optional[DatasetMetadata]:
        ...
        return mongo_service.find_one(
            self.collection_name, {"dataset_id": openml_id}, self.model_class
        )

    async def async_find_by_openml_id(
        self, openml_id: str
    ...
    def find_by_tags(
        ...
        return mongo_service.find(self.collection_name, query, self.model_class)
    ...
    async def async_find_by_tags(
        ...
        return await mongo_service.async_find(
            self.collection_name, query, self.model_class
        )
...
class DatasetVersionRepository(MongoRepository[DatasetVersion]):
    ...
    def find_by_version_id(self, version_id: str) -> Optional[DatasetVersion]:
        ...
        return mongo_service.find_one(
            self.collection_name, {"version_id": version_id}, self.model_class
        )

    async def async_find_by_version_id(
        self, version_id: str
    ...
    async def async_find_versions_for_dataset(
        self, dataset_id: str
    ) -> List[DatasetVersion]:
    ...
    async def async_get_latest_version(
        self, dataset_id: str
    ) -> Optional[DatasetVersion]:
...
class DatasetTrainingResultRepository(MongoRepository[DatasetTrainingResult]):
    ...
    def find_by_training_id(self, training_id: str) -> Optional[DatasetTrainingResult]:
        ...
        return mongo_service.find_one(
            self.collection_name, {"training_id": training_id}, self.model_class
        )

    async def async_find_by_training_id(
    ...
    async def async_find_results_for_agent(
        self, agent_id: str
    ) -> List[DatasetTrainingResult]:
    ...
    async def async_find_results_for_dataset(
        self, dataset_id: str
    ) -> List[DatasetTrainingResult]:
