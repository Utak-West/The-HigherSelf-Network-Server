...
class MongoRepository(Generic[T]):
    ...
    def find_by_id(self, model_id: str) -> Optional[T]:
        """Find a model by ID."""
        return mongo_service.find_model_by_id(
            self.collection_name, model_id, self.model_class
        )

    async def async_find_by_id(self, model_id: str) -> Optional[T]:
        """Find a model by ID asynchronously."""
        return await mongo_service.async_find_model_by_id(
            self.collection_name, model_id, self.model_class
        )

    def find_all(self, limit: int = 0, skip: int = 0) -> List[T]:
        """Find all models in the collection."""
        return mongo_service.find_models(
            self.collection_name, {}, self.model_class, limit, skip
        )

    async def async_find_all(self, limit: int = 0, skip: int = 0) -> List[T]:
        """Find all models in the collection asynchronously."""
        return await mongo_service.async_find_models(
            self.collection_name, {}, self.model_class, limit, skip
        )

    def find_by_filter(
        self, filter_dict: Dict[str, Any], limit: int = 0, skip: int = 0
    ) -> List[T]:
        """Find models matching a filter."""
        return mongo_service.find_models(
            self.collection_name, filter_dict, self.model_class, limit, skip
        )

    async def async_find_by_filter(
        self, filter_dict: Dict[str, Any], limit: int = 0, skip: int = 0
    ) -> List[T]:
    ...
    async def async_delete_by_id(self, model_id: str) -> int:
        ...
        return await mongo_service.async_delete_one(
            self.collection_name, {"id": model_id}
        )
...
class WorkflowInstanceRepository(MongoRepository[WorkflowInstanceDocument]):
    ...
    async def async_find_by_workflow_id(
        self, workflow_id: str
    ) -> List[WorkflowInstanceDocument]:
...
class TaskRepository(MongoRepository[TaskDocument]):
    ...
    def find_by_workflow_instance_id(
    ...
    async def async_find_by_workflow_instance_id(
        self, workflow_instance_id: str
    ) -> List[TaskDocument]:
...
class AgentCommunicationRepository(MongoRepository[AgentCommunicationDocument]):
    ...
    async def async_find_by_target_agent_id(
        self, target_agent_id: str
    ) -> List[AgentCommunicationDocument]:
...
class ApiIntegrationRepository(MongoRepository[ApiIntegrationDocument]):
    ...
    async def async_find_by_platform(
        self, platform: str
    ) -> List[ApiIntegrationDocument]:
