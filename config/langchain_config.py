...
class LangChainConfig(BaseSettings):
    ...
    vector_store_path: str = Field(
        default="./vector_store", env="LANGCHAIN_VECTOR_STORE_PATH"
    )
    embedding_model: str = Field(
        default="text-embedding-ada-002", env="LANGCHAIN_EMBEDDING_MODEL"
    )

    # Memory settings
    memory_max_tokens: int = Field(default=2000, env="LANGCHAIN_MEMORY_MAX_TOKENS")
    conversation_ttl: int = Field(
        default=86400, env="LANGCHAIN_CONVERSATION_TTL"
    )  # 24 hours

    # Security settings
    max_input_length: int = Field(default=10000, env="LANGCHAIN_MAX_INPUT_LENGTH")
    enable_input_validation: bool = Field(
        default=True, env="LANGCHAIN_ENABLE_INPUT_VALIDATION"
    )
    enable_output_filtering: bool = Field(
        default=True, env="LANGCHAIN_ENABLE_OUTPUT_FILTERING"
    )
