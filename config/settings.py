...
# Since we're now using Pydantic v2 as specified in requirements.txt
from pydantic import AnyHttpUrl, Field, field_validator
from pydantic_settings import BaseSettings

# Explicitly set to True since we're using Pydantic v2
PYDANTIC_V2 = True
...
class NotionSettings(BaseSettings):
    ...
    api_token: str = Field(
        default="test_notion_token_" + "x" * 40, env="NOTION_API_TOKEN"
    )
...
class ServerSettings(BaseSettings):
    ...
    webhook_secret: str = Field(default="test_webhook_secret", env="WEBHOOK_SECRET")
