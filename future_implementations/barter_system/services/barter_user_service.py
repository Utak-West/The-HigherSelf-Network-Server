...
from models.barter_models import (BarterNotificationPreference, BarterProfile,
                                  BarterUserProfile, LanguageCode,
                                  NotificationFrequency, VerificationStatus)
...
class BarterUserService:
    ...
    async def create_user_profile(
        ...
        try:
            user_profile = BarterUserProfile(
                ...
                or self._get_default_privacy_settings(),
            ...
            await redis_service.async_set(
                cache_key, user_profile.model_dump_json(), ex=self.cache_ttl
            )
    ...
    async def link_barter_profile(self, user_id: str, barter_profile_id: UUID) -> bool:
        """
        ...
        try:
            ...
            if user_profile:
                logger.info(
                    f"Linked barter profile {barter_profile_id} to user {user_id}"
                )
                return True
    ...
    async def update_verification_status(
        ...
        try:
            ...
            if user_profile:
                ...
                await redis_service.async_set(
                    verification_cache_key, status.value, ex=self.verification_cache_ttl
                )
    ...
    async def update_notification_preference(
        ...
        try:
            ...
            await redis_service.async_set(
                cache_key, preferences_data, ex=self.cache_ttl
            )
    ...
    async def _create_default_notification_preferences(
        ...
        await redis_service.async_set(cache_key, preferences_data, ex=self.cache_ttl)
