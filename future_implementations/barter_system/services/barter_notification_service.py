...
class BarterNotificationService:
        ...
        **kwargs,
        ...
        try:
            ...
            notification = BarterNotification(
                ...
                priority=priority
                or template.get("priority", NotificationPriority.NORMAL),
                title=custom_title or template.get("title", "Barter Notification"),
                message=custom_message
                or template.get("message", "You have a new barter notification"),
                data=data,
                channels=channels
                or template.get("channels", [NotificationChannel.IN_APP]),
            ...
            await redis_service.async_set(
                cache_key, notification.model_dump_json(), ex=self.cache_ttl
            )
            ...
            logger.info(
                f"Created barter notification: {notification.id} for user {recipient_id}"
            )
    ...
    async def _send_email_notification(self, notification: BarterNotification):
        ...
        logger.info(
            f"Email notification sent to {notification.recipient_id}: {notification.title}"
        )
    ...
    async def _send_sms_notification(self, notification: BarterNotification):
        ...
        logger.info(
            f"SMS notification sent to {notification.recipient_id}: {notification.title}"
        )
    ...
    async def _send_push_notification(self, notification: BarterNotification):
        ...
        logger.info(
            f"Push notification sent to {notification.recipient_id}: {notification.title}"
        )
    ...
    async def _send_in_app_notification(self, notification: BarterNotification):
        ...
        logger.info(
            f"In-app notification created for {notification.recipient_id}: {notification.title}"
        )
    ...
    async def _send_webhook_notification(self, notification: BarterNotification):
        ...
        logger.info(
            f"Webhook notification sent for {notification.recipient_id}: {notification.title}"
        )
    ...
    async def get_user_notifications(
        ...
        try:
            ...
            notification_ids = await redis_service.async_lrange(
                user_notifications_key, 0, limit - 1
            )
            ...
            for notification_id in notification_ids:
                ...
                if notification_data:
                    notification = BarterNotification.model_validate_json(
                        notification_data
                    )
    ...
    async def notify_new_match(
        self, match: BarterMatch, requester_id: str, listing: BarterListing
    ):
    ...
    async def notify_match_accepted(
        self, match: BarterMatch, provider_name: str, service_type: str
    ):
        ...
        await self.create_notification(
            ...
            data={"provider_name": provider_name, "service_type": service_type},
            related_match_id=match.id,
        )
    ...
    async def notify_transaction_created(self, transaction: BarterTransaction):
        ...
        await self.create_notification(
            ...
            data={
                ...
                "service_type": transaction.requester_service.get(
                    "category", "service"
                ),
            },
            related_transaction_id=transaction.id,
    ...
    async def notify_transaction_completed(self, transaction: BarterTransaction):
        ...
        await self.create_notification(
            ...
            data={"partner_name": transaction.requester_id},
            related_transaction_id=transaction.id,
        )
        ...
        await self.create_notification(
            recipient_id=transaction.requester_id,
            notification_type=NotificationType.TRANSACTION_COMPLETED,
            data={"partner_name": transaction.provider_id},
            related_transaction_id=transaction.id,
        )
